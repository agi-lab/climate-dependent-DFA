...
GDP_projections_dat_path <- "Data/SSP_scenarios.csv" ## GDP projections data path
Pop_projections_dat_path <- "Data/AU_population_projections.xlsx" ## Population projections data path
...
EM_dat_path <- "Data/EM_DAT_AU.xlsx" ## EM-DAT loss data path
```
...
australia_shape <- readOGR("Data/AUS_2021_AUST_GDA2020.shp") ## Enter the shape file for Australia
...
GI_fins_stats_input  <- "Data/Annual general insurance institution-level statistics database March 2005 to June 2023.xlsx"  ## Enter the file path for institution-level financial statistics from APRA
...
############################################################
tas_ERA5_hist_dat <- transform_nc_tas_mm_func(australia_shape, nc_tas_data_ERA5_hist) %>%
...
ta_ERA5_hist_dat <- transform_nc_tas_mm_func(australia_shape, nc_ta_data_ERA5_hist) %>% rename(Air_temperature = Temperature) %>% filter(Year >= 1950 & Year <= 2023)
```
...
```{r Combine the historical FWI data}
...
FWI_ref_data <- rbind(FWI_40_49_data, FWI_50_59_data, FWI_60_65_data ) %>% rename(Lat = x, Lon = y)
```
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Climate/SST_AU_basin.pdf")
## Plot SST:
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Liabilities/Exposure_vs_Population.pdf")
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/MeanGACR.pdf")
