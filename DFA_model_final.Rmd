...
- Section [Sensitivity analysis]: This section presents the sensitivity analysis results shown in the appendix of the paper. 
...
pacman::p_load(
  ...
  plotly, GDPuc, colorspace, Rmpfr, cvar, VGAM, data.table, openxlsx, AER, viridis, MASS
)
```

```{r Define color sclae used for plotting}
### Define a series of color scale
viridis_color_scale <- viridis(10)
inferno_color_scale <- viridis(20, option = "inferno")

green_col <- viridis_color_scale[6]
purple_col <- inferno_color_scale[4]
orange_col <- viridis_color_scale[10]
red_col <- inferno_color_scale[11]
```

```{r}
# viridis_colors <- viridis(20, option = "inferno")
# 
# # Convert HEX to RGB and then to grayscale using luminance formula
# grayscale_values <- col2rgb(viridis_colors)
# luminance <- 0.299 * grayscale_values[1, ] +
#              0.587 * grayscale_values[2, ] +
#              0.114 * grayscale_values[3, ]
# 
# # Normalize and plot
# plot(luminance, type = "h", lwd = 10, col = viridis_colors,
#      main = "Viridis Grayscale Brightness", xlab = "Color Index", ylab = "Luminance")
```
...
calc_surplus_func_full_cycle <- function(Risk_free_weight, 
                              ...
                              k3_param = k3_default, 
                              assumption_alternative = FALSE, Div_payout = 0, solvency_ratio_steady = 1, Div_limit = 0.5){
     ...
     brown_return <- equities_output_brown[[scenario_index]][1:horizon, i]
     other_return <- equities_output[[scenario_index]][1:horizon, i]
     ...
     for(k in 2:(length(Premium_gross_CAT)+1)){
         ...
         P_gross <- Premium_gross_CAT[k-1] + Premium_gross_Non_CAT[k-1] + Premium_gross_Non_CAT_other[k-1]
         P_net <- Premium_gross_CAT[k-1] + Premium_gross_Non_CAT[k-1] + Premium_gross_Non_CAT_other[k-1] - P_re_sim
         ## Calculate the dividend payout from primary insurers:
         capital_exDiv <- (capital_projections[k-1] + P_net)*(1 + investment_return[k-1]) - claims_cost_total[k-1]
         net_profit <- capital_exDiv - capital_projections[k-1]
         Div <- max(min(net_profit * Div_limit, dividends_payout_func(K_exDiv = capital_exDiv, P_t = P_gross, S_0 = solvency_ratio_steady, k3 = Div_payout)), 0)
         ## Calculate the dividend payout from reinsurers:
         ...
         capital_projections[k] <- ifelse(capital_projections[k-1] <= 0, 0, capital_exDiv - Div)
...
calc_reinsurance_recoveries_norm_stationary <- function(horizon, market_cap, reinsurance_excess, reinsurance_limit_multiple, DFA_list){
     Re_recoveries_mat <- matrix(NA, nrow = horizon, ncol = n.sims)
     CAT_loss_mat <- matrix(NA, nrow = horizon, ncol = n.sims)
     for(i in 1:n.sims){
     ## Get the gross CAT losses:
     CAT_losses_gross <- DFA_list$Liabilities_module$CAT_normalised_loss[1:horizon, i]*market_cap
     ## Get the net CAT losses:
     Re_recoveries_mat[, i] <-  pmax(pmin(CAT_losses_gross-reinsurance_excess, reinsurance_excess*reinsurance_limit_multiple), 0)
     CAT_loss_mat[, i] <- CAT_losses_gross - pmax(pmin(CAT_losses_gross-reinsurance_excess, reinsurance_excess*reinsurance_limit_multiple), 0)
     }
...
plot_quantiles_scenario <- function(quantiles_list, forecast_periods, var_name, main, y_range, colour_list, horizon, transparency, label_percents = F, perc_step = 0.05, LOG = FALSE, cex.main.choice = 1, cex.lab.choice = 1, cex.axis.choice = 1, xlab.name = "Years", mean_lwd = 2, quantiles_lwd = 1.5){
    if(label_percents == T){
    # Plotting
    plot(forecast_periods, quantiles_list[[1]][2, 1:horizon], type = "n", ylim = y_range,
         xlab = xlab.name, ylab = var_name, main = main, yaxt = "n", cex.main = cex.main.choice, cex.lab = cex.lab.choice, cex.axis = cex.axis.choice)
    axis(2, at = seq(y_range[1], y_range[2], by = perc_step), labels = paste0(seq(y_range[1], y_range[2], by = perc_step) * 100, '%'), las = 1)
    } else{
        plot(forecast_periods, quantiles_list[[1]][2, 1:horizon], type = "n", ylim = y_range,
         xlab = xlab.name, ylab = var_name, main = main, cex.main = cex.main.choice, cex.lab = cex.lab.choice, cex.axis = cex.axis.choice)
    }
    if(LOG == FALSE){
    for(j in 1:length(quantiles_list)){
        ...
        # Add quantile lines
        lines(forecast_periods, quantiles_list[[j]][2, 1:horizon], col = colour, lwd = mean_lwd, lty = 1) # 50% quantile (median)
        lines(forecast_periods, quantiles_list[[j]][1, 1:horizon], col = colour, lwd = quantiles_lwd, lty = 2) # Lower quantile
        lines(forecast_periods, quantiles_list[[j]][3, 1:horizon], col = colour, lwd = quantiles_lwd, lty = 2) # Upper quantile
    }} else{
        for(j in 1:length(quantiles_list)){
        colour = colour_list[[j]]
        # Fill areas between quantiles
        ...
        # Add quantile lines
        lines(forecast_periods, log(quantiles_list[[j]][2, 1:horizon]), col = colour, lwd = mean_lwd, lty = 1) # 50% quantile (median)
        lines(forecast_periods, log(quantiles_list[[j]][1, 1:horizon]), col = colour, lty = 2, lwd = quantiles_lwd) # Lower quantile
        lines(forecast_periods, log(quantiles_list[[j]][3, 1:horizon]), col = colour, lty = 2, lwd = quantiles_lwd) # Upper quantile
            }
        }
}

## Alternative function for plotting quantiles:
plot_quantiles_scenario_2 <- function(quantiles_list, forecast_periods, var_name, main, y_range, colour_list, horizon, transparency, label_percents = F, lty_mean = 1, lwd_mean = 2, step_plot_perc = 0.05, quantile_lines = T, cex.main.choice = 1, cex.lab.choice = 1, cex.axis.choice = 1, xlab.name = "Years", lwd_quantiles = 1.5){
    if(label_percents == T){
    # Plotting
    plot(forecast_periods, quantiles_list[[1]][2, 1:horizon], type = "n", ylim = y_range,
         xlab = xlab.name, ylab = var_name, main = main, yaxt = "n", cex.main = cex.main.choice, cex.lab = cex.lab.choice, cex.axis = cex.axis.choice)
    ...
    for(j in 1:length(quantiles_list)){
        ...
        lines(forecast_periods, quantiles_list[[j]][1, 1:horizon], col = colour, lwd = lwd_quantiles, lty = 2) # Lower quantile
        lines(forecast_periods, quantiles_list[[j]][3, 1:horizon], col = colour, lwd = lwd_quantiles, lty = 2) # Upper quantile
...
multi_func_withMed2 <- function(x, quantile_low = 0.05, quantile_high = 0.95){
    c(quantile_05 = quantile(x, quantile_low), median = median(x), quantile_95 = quantile(x,quantile_high), mean = mean(x))
}
...
```{r Define simulation functions under the stationary cases}
## Simulation for CAT counts: ----------------
simulate_Pois_GLM_stationary <- function(model, horizon = forecast_horizon){
    pred_freq <- mean(predict(model, type = "response"))
    simulate_response <- rpois(horizon, pred_freq)
    return(list(Count_yy = simulate_response))
}

## Simulation for CAT severity: ---------------
sim_LN_CAT_losses_func_stationary <- function(LN_model, CAT_count, horizon = forecast_horizon){
    pred_mu_LN <- mean(predict(LN_model, type = "response", what = "mu"))
    pred_sigma_LN <- mean(predict(LN_model, type = "response", what = "sigma"))
    #CAT_loss_individual <- list()
    CAT_loss_aggregate <- c()
    ## Obtain the simulations
    for(i in 1:horizon){
        ## Simulate the LN losses:
        sim_LN <- rlnorm(CAT_count[i], meanlog  = pred_mu_LN, sdlog  = pred_sigma_LN)
        ## Simulate the CAT losses:
        CAT_loss <- sim_LN 
        #CAT_loss_individual[[i]] <- CAT_loss
        CAT_loss_aggregate[i] <- sum(CAT_loss)
...
```{r Fit stationary model for flood frequency}
glm_Flood_freq_stationary <- glm(CAT_count ~ 1, family = poisson(link = "log"), data = CAT_food_ICA_count)
```
...
```{r Fit stationary model for flood severity}
glm_Flood_sev_stationary <- gamlss(formula =  CAT_loss_normalised_GDP ~ 1, data = CAT_food_ICA_loss, family = LOGNO(mu.link="identity", sigma.link="log"), trace = FALSE)
...
```{r Fit stationary frequency model bushfire}
glm_Bush_freq_stationary <- glm(CAT_count ~ 1, data = CAT_Bushfire_ICA_count, family = poisson(link = "log"))
```
...
```{r Fit stationary severity model bushfire}
glm_Bush_sev_stationary <- gamlss(formula =  CAT_loss_normalised_GDP ~ 1, data = CAT_Bushfire_ICA_loss, family = LOGNO(mu.link="identity", sigma.link="log"), trace = FALSE)
```
...
```{r Fit stationary frequency model TC}
glm_Cyc_freq_stationary <- glm(CAT_count ~ 1, data = CAT_Cyclone_ICA_count, family = poisson(link = "log"))
```
...
```{r Fit stationary severity model TC}
glm_Cyc_sev_stationary <- gamlss(formula =  CAT_loss_normalised_GDP ~ 1, data = CAT_Cyclone_ICA_loss_mm, family = LOGNO(mu.link="identity", sigma.link="log"), trace = FALSE)
```
...
```{r Fit stationary frequency model Storm}
glm_Storm_freq_stationary <- glm(CAT_count ~ 1, data = CAT_Storm_ICA_count, family = poisson(link = "log"))
```
...
```{r Fit stationary severity model Storm}
glm_Storm_sev_stationary <- gamlss(formula =  CAT_loss_normalised_GDP ~ 1, data = CAT_Storm_ICA_loss_mm, family = LOGNO(mu.link="identity", sigma.link="log"), trace = FALSE)
```
...
```{r Fit a stationary ECL frequency model}
glm_ECL_freq_stationary <- glm(CAT_count ~ 1, data = CAT_ECL_ICA_count, family = poisson(link = "log"))
```
...
```{r Fit a stationary ECL severity model}
glm_ECL_sev_stationary <- glm_ECL_sev_0 
```
...
```{r Hail storm frequency model stationary}
glm_HS_freq_stationary <- glm(CAT_count ~ 1, data = CAT_HL_ICA_count, family = poisson(link = "log"))
glm_HS_freq_stationary
```
...
glm_HS_sev_stationary <- glm_HS_sev_0
...
## Dispersion test for selected frequency models for all hazard types


```{r Dispersion test for all hazards frequency model}
print("============= Flood frequency model (Poisson) ==========================")
dispersiontest(glm_Flood_freq_3, trafo = NULL, alternative = "two.sided")
print("============= Bushfire frequency model (Poisson) ==========================")
dispersiontest(glm_Bush_freq_1, trafo = NULL, alternative = "two.sided")
print("============= Tropical Cyclones frequency model (Poisson) ==========================")
dispersiontest(glm_Cyc_mm_freq_3, trafo = NULL, alternative = "two.sided")
#print("============= Storms frequency model (Poisson) ==========================")
#dispersiontest(glm_Storm_mm_freq_1, trafo = NULL, alternative = "two.sided")
print("============= ECL frequency model (Poisson) ==========================")
dispersiontest(glm_ECL_freq_2, trafo = NULL, alternative = "two.sided")
print("============= Hailstorms frequency model (Poisson) ==========================")
dispersiontest(glm_HS_freq, trafo = NULL, alternative = "two.sided")
```
...
Fit a interest rate model under the stationary setting:

- An AR(1) model is fitted, which can be viewed as a special case of the generalised CIR model in Kaufmann et al. (2000) when the parameter $g$ is set to zero.

```{r Stationary interest rate model}
SR_mod_AR1_stationary <- sarima(AU_interest_rates_GDP_potential$Short_rates_real, 1, 0, 0, details = FALSE)
SR_mod_AR1_stationary$ttable
## Plot the forecast: ---------
real_rates_forecast_stationary <- sarima.for(AU_interest_rates_GDP_potential$Short_rates_real, n.ahead = 38, 1, 0, 0, plot = TRUE, xlab = "Periods", main = "Forecast of real rates (stationary)")
```
...
```{r Save calibrated non-CAT model}
#non_catastrophe_loss_model_list <- list(Non_CAT_PM = tw_PM_0)

```

Under the stationary case, we assume the exposure follows the AR(1) process as per Kaufmann et al (2000): 


```{r Fit stationary exposure growth model}
## Derive growth rate of exposure: -----
Exposure_growth_rates <- as.numeric(na.omit(GI_exposure_pop_data$Exposure/lag(GI_exposure_pop_data$Exposure) - 1))
## Fit a AR(1) model on the exposure growth rates: -----
lm_exposure_growth_stationary <- sarima(Exposure_growth_rates, 1, 0, 0, details = FALSE)
lm_exposure_growth_stationary$ttable
## Forecast of exposure growth rates: -----
exposure_growth_forecast_stationary <- sarima.for(Exposure_growth_rates, n.ahead = 38, 1, 0, 0, details = FALSE, xlab = "Periods", main = "Forecast of exposure growth rates")
...
### Calibration of stationary equiry returns model

```{r Calibrate the stationary equity return model}
lm_OP_TR_stationary <- lm(Total_return ~ 1, data = TR_all_OP_ord_shares_yy_data, subset = Year %nin% c(2008, 2009, 2012))
```
...
col_list <- c(green_col, orange_col, red_col, purple_col) ## Define the colour used for plotting the simulations
```
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Climate/p_values_NS_residuals.pdf")
plt <- barplot(p_vals_shapiro_tas, col='steelblue', xaxt="n", main = "p-values for Shapiro-Wilk test (NS temperature residuals)", xlab = "Models", ylab = "p-values")
...
```{r Calculate noise and standard deviations for TA residuals}
ta_noise_mat <- matrix(NA, nrow = nrow(ta_ERA5_ensemble_hist_dat), ncol = length(Models_ensemble_ta))
ta_quantile_sd <- c()
ta_hist_sd <- c()
for(i in 1:length(Models_ensemble_ta)){
        mod <- Models_ensemble_ta[i]
        lm_ta <- lm(sort(Air_temperature) ~ sort(ta_ERA5_ensemble_hist_dat[, mod]), data = ta_ERA5_ensemble_hist_dat)
        ta_model_correct <- lm_ta$coefficients[1] +  lm_ta$coefficients[2]*ta_ERA5_ensemble_hist_dat[, mod]
        ## Collect the noise of projections: 
        noise_ta <- ta_ERA5_ensemble_hist_dat$Air_temperature - ta_model_correct
        ta_noise_mat[, i] <- noise_ta
        ## Collect the quantile standard deviations:
        ta_quantile_sd[i] <- summary(lm_ta)$sigma
        ## Collect the historical standard deviations:
        ta_hist_sd[i] <- sd(noise_ta)
    }
```

```{r Perform shapiro normality test for residuals (Air temperature)}
p_vals_shapiro_ta <- c()
for(i in 1:ncol(ta_noise_mat)){
    shapiro_test_stats <- shapiro.test(ta_noise_mat[, i])
    p_vals_shapiro_ta[i] <- shapiro_test_stats$p.value
}
pdf("/Users/jimli/Documents/DFA codes/Plotting/Climate/p_values_AirTemperature_residuals.pdf")
plt <- barplot(p_vals_shapiro_ta, col='steelblue', xaxt="n", main = "p-values for Shapiro-Wilk test (Air temperature residuals)", xlab = "Models", ylab = "p-values")
text(plt, par("usr")[3], labels = Models_ensemble_ta, srt = 60, adj = c(1.1,1.1), xpd = TRUE, cex=0.6)
abline(h = 0.05, lty = 2, col = "red")
text("")
#dev.off()
```
...
```{r Calculate noise and standard deviations for SST residuals}
SST_noise_mat <- matrix(NA, nrow = nrow(SST_ERA5_ensemble_hist_dat), ncol = length(Models_ensemble_SST))
SST_quantile_sd <- c()
SST_hist_sd <- c()
for(i in 1:length(Models_ensemble_SST)){
        mod <- Models_ensemble_SST[i]
        lm_SST <- lm(sort(SST_mean_mm) ~ sort(SST_ERA5_ensemble_hist_dat[, mod]), data = SST_ERA5_ensemble_hist_dat)
        SST_model_correct <- lm_SST$coefficients[1] +  lm_SST$coefficients[2]*SST_ERA5_ensemble_hist_dat[, mod]
        ## Collect the noise of projections: 
        noise_SST <- SST_ERA5_ensemble_hist_dat$SST_mean_mm- SST_model_correct
        SST_noise_mat[, i] <- noise_SST
        ## Collect the quantile standard deviations:
        SST_quantile_sd[i] <- summary(lm_SST)$sigma
        ## Collect the historical standard deviations:
        SST_hist_sd[i] <- sd(noise_SST)
    }
```


```{r QQPlot of SST residuals for each model, fig.height = 7}
par(mfrow = c(3, 3))
for(i in 1:ncol(SST_noise_mat)){
    qqplot(qnorm(ppoints(nrow(SST_noise_mat)), mean = 0, sd = SST_hist_sd[i]), SST_noise_mat[, i], xlab = "Theorectical Quantiles (Normal)", ylab = "Empirical Quantiles", main = "Q-Q plot of SST residuals")
    abline(0, 1, col = "red", lwd = 2)
}
```
...
```{r Perform shapiro normality test for residuals (SST)}
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Climate/p_values_SST_residuals.pdf")
plt <- barplot(p_vals_shapiro_SST_grad, col='steelblue', xaxt="n", main = "p-values for Shapiro-Wilk test (SST residuals)", xlab = "Models", ylab = "p-values")
...
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Climate/p_values_rx5day_residuals.pdf")
...
### Define additional stationary input parameters

```{r Define inputs under the stationary setting}
## Exposure forecasts -----------
exposure_growth_pred_stationary <- as.numeric(sarima.for(Exposure_growth_rates, n.ahead = forecast_horizon, 1, 0, 0, plot = FALSE)$pred)

Exposure_forecasts_stationary <- Exposure_start * cumprod(1 + exposure_growth_pred_stationary)

## GDP forecasts -----------

GDP_growth_forecasts_stationary <- as.numeric(sarima.for(AU_interest_rates_GDP_potential$GDP_potential_growth, n.ahead = forecast_horizon, 1, 0, 0, plot = FALSE)$pred)

GDP_forecasts_stationary <- Real_GDP_start * cumprod(1 + GDP_growth_forecasts_stationary)

## Interest rates:-----------

mu_SR_stationary <- SR_mod_AR1_stationary$ttable["xmean" , "Estimate"]
phi_SR_stationary <- SR_mod_AR1_stationary$ttable["ar1" , "Estimate"]
sigma_SR_stationary <- sqrt(SR_mod_AR1_stationary$fit$sigma2)
SR_start <- last(AU_interest_rates_GDP_potential$Short_rates_real)

## Equity returns: -------------

mu_TR_stationary <- summary(lm_OP_TR_stationary)$coefficients[1]
sigma_TR_stationary <- summary(lm_OP_TR_stationary)$sigma
```
...
```{r Save simulation results of environmental variables}
...
Real_GDP_pred_adj_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
...
for(i in 1:n.sims){
    ...
    Real_GDP_pred_adj_mat[, i] <- Real_GDP_pred_adj_growth
}
...
Real_GDP_pred_adj_list[[s]] <- Real_GDP_pred_adj_mat
}
```

```{r Save simulation results of asset module}
asset_module_output_list <- list(general_portfolio = equity_TR_excess_adj_OP_list, brown_portfolio = equity_TR_excess_adj_OP_brown_list, adjusted_GDP_growth = Real_GDP_pred_adj_list)
save(asset_module_output_list, file = "results/Simulated_results_asset_module_sims10000.Rdata")
```
...
### Perform the stationary simulations





```{r Write a function to simulate DFA components under the stationary case}

simulate_DFA_components_stationary_func <- function(n.sims){
    ...
    for(i in 1:n.sims){
            ...
            ## Simulate the baseline interest rates:
            simulated_SR <- simulate_SR_AR1(intercept = mu_SR_stationary, phi = phi_SR_stationary, sigma = sigma_SR_stationary, n = (forecast_horizon + 1), start_value = SR_start)[-1]
            ...
            ########################################################################
            ## Calculate the climate-adjusted inflation rates:
             Inflation_rates_sim <- simulateAR1_Inflation_Clim(mu = inflation_mod_bench$coef[2], phi = inflation_mod_bench$coef[1], sigma = sqrt(inflation_mod_bench$sigma2), n = forecast_horizon, start_value = inflation_rate_start, start_value_CPI = CPI_start, Climate_pressure = rep(0, forecast_horizon))
            ...
            ## Flood
            Flood_count_sim <- simulate_Pois_GLM_stationary(glm_Flood_freq_stationary, horizon = forecast_horizon)$Count_yy
            Flood_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_Flood_sev_stationary, Flood_count_sim, horizon = forecast_horizon)

            ## Bushfire:
            BF_count_sim <- simulate_Pois_GLM_stationary(glm_Bush_freq_stationary, horizon = forecast_horizon)$Count_yy
            BF_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_Bush_sev_stationary, BF_count_sim, horizon = forecast_horizon)
            
            ## TC
            TC_count_sim <- simulate_Pois_GLM_stationary(glm_Cyc_freq_stationary, horizon = forecast_horizon)$Count_yy
            TC_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_Cyc_sev_stationary, TC_count_sim, horizon = forecast_horizon)
            
            ## Storm
            Storm_count_sim <- simulate_Pois_GLM_stationary(glm_Storm_freq_stationary, horizon = forecast_horizon)$Count_yy
            Storm_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_Storm_sev_stationary, Storm_count_sim, horizon = forecast_horizon)
            
            ## ECL
            ECL_count_sim <- simulate_Pois_GLM_stationary(glm_ECL_freq_stationary, horizon = forecast_horizon)$Count_yy
            ECL_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_ECL_sev_stationary, ECL_count_sim, horizon = forecast_horizon)
            
            ## Hailstorm
            HS_count_sim <- simulate_Pois_GLM_stationary(glm_HS_freq_stationary, horizon = forecast_horizon)$Count_yy
            HS_total_norm_loss_sim <- sim_LN_CAT_losses_func_stationary(glm_HS_sev_stationary, HS_count_sim, horizon = forecast_horizon)
             ########################################################################
            #### #### #### #### Liabilities module #### #### #### #### #### 
            ########################################################################
            # CAT losses:
            ## Calculate normalised aggregate losses
             sim_CAT_losses <- Flood_total_norm_loss_sim + BF_total_norm_loss_sim + TC_total_norm_loss_sim + Storm_total_norm_loss_sim + ECL_total_norm_loss_sim + HS_total_norm_loss_sim
            ## Adjust the CAT loss for wealth exposure, and cap the simulated losses at the exposure:
            sim_CAT_losses_GDP_adj <- pmin(sim_CAT_losses*GDP_forecasts_stationary/Real_GDP_start, GDP_forecasts_stationary)
            sim_CAT_damage_ratio <- sim_CAT_losses_GDP_adj/GDP_forecasts_stationary
            ## Also adjust the CAT loss for price level:
            sim_CAT_losses_full_adj <- sim_CAT_losses_GDP_adj*Inflation_rates_sim$CPI/CPI_start
            ...
            ## Non-CAT losses for property, motor, fire and ISR: 
            sim_Non_CAT_avg <- sim_tweedie(mu = rep(exp(model_nonCAT$coefficients), forecast_horizon), phi = summary(model_nonCAT)$dispersion, power = power_tw)
            sim_Non_CAT_agg <- sim_Non_CAT_avg*Exposure_forecasts_stationary
            sim_Non_CAT_agg_withInflation <- sim_Non_CAT_agg*Inflation_rates_sim$CPI/CPI_start
             ## Non-CAT losses for other BoLs:
            sim_Non_CAT_other_avg <- sim_tweedie(mu = rep(exp(model_nonCAT_other$coefficients), forecast_horizon), phi = summary(model_nonCAT_other)$dispersion, power = power_tw)
            sim_Non_CAT_other_agg <- sim_Non_CAT_other_avg*Exposure_forecasts_other
    ...
    ## Define a list to store all DFA components:
    DFA_components_list <- list(MEV_module = MEV_list,
                                Hazard_module_sev = Hazard_severity_list,
                                Liabilities_module = Liabilities_list)
   
     return(DFA_components_list)
}

```

```{r Simulations of DFA under stationary case}
DFA_components_stationary <- simulate_DFA_components_stationary_func(n.sims)
```


```{r Save simulation results of environmental variables under stationary case}
save(DFA_components_stationary, file = "Simulated_results_stationary_DFA_components_sims10000.Rdata")
```
...
```{r Save expected CAT claims cost}
expected_CAT_norm_claims_list <- list(Flood = Flood_loss_pred, Bushfire = Bushfire_loss_pred,
                                 TC = TC_loss_pred_dat_yy, Storm = Storm_loss_pred_dat_yy,
                                 ECL = ECL_loss_pred_dat_yy, HS = HS_loss_pred_dat_yy)
save(expected_CAT_norm_claims_list, file = "results/Expected_CAT_claims_cost_normalised.Rdata")
```
...
```{r Save variance of CAT claims cost}
var_CAT_norm_claims_list <- list(Flood = Flood_var_mat, Bushfire = Bushfire_var_mat,
                                 TC = TC_var_mat, Storm = Storm_var_mat,
                                 ECL = ECL_var_mat, HS = HS_var_mat)
save(var_CAT_norm_claims_list, file = "results/Variance_CAT_claims_cost_normalised.Rdata")
```
...
```{r}
#Hazard_loss_pred_dat
#Hazard_var_agg_mat
gross_premium_CAT_norm_mat 
```
...
### Premium module (stationary assumption)

#### Gross premium (stationary)


```{r Calculate expected inflation rates and CPI (under stationary assumption)}
## Calculate inflation rates: ---------
Inflation_rates_expected_stationary <- as.numeric(predict(inflation_mod_bench, n.ahead = forecast_horizon)$pred)

## Calculate expected CPI: ----------
CPI_expected_stationary <- c()
CPI_expected_stationary[1] <- CPI_start
    for(t in 2:(forecast_horizon+1)){
        CPI_expected_stationary[t] <- CPI_expected_stationary[t-1]*(1 + Inflation_rates_expected_stationary[t-1])
    }
```

```{r}
#plot(Inflation_rates_expected_stationary, type = "l")
```


```{r Calculate the hazard risk premiums under the stationary setting}

 ## Flood: ----------
Flood_freq_stationary <- mean(predict(glm_Flood_freq_stationary, type ="response"))
Flood_sev_mu_stationary <- mean(predict(glm_Flood_sev_stationary, type = "response", what = "mu"))
Flood_sev_sigma_stationary <- mean(predict(glm_Flood_sev_stationary, type = "response", what = "sigma"))
Flood_sev_stationary <- exp(Flood_sev_mu_stationary + 0.5*Flood_sev_sigma_stationary^2)
Flood_loss_stationary  <- Flood_sev_stationary*Flood_freq_stationary
#Flood_loss_pred_dat/Flood_loss_stationary

## Bushfire: --------
Bush_freq_stationary <- mean(predict(glm_Bush_freq_stationary, type ="response"))
Bush_sev_mu_stationary <- mean(predict(glm_Bush_sev_stationary, type = "response", what = "mu"))
Bush_sev_sigma_stationary <- mean(predict(glm_Bush_sev_stationary, type = "response", what = "sigma"))
Bush_sev_stationary <- exp(Bush_sev_mu_stationary + 0.5*Bush_sev_sigma_stationary^2)
Bush_loss_stationary  <- Bush_sev_stationary*Bush_freq_stationary
## Check: 
#Bushfire_loss_pred_dat/Bush_loss_stationary

## Tropical cyclone: -------
Cyc_freq_stationary <- mean(predict(glm_Cyc_freq_stationary, type ="response"))
Cyc_sev_mu_stationary <- mean(predict(glm_Cyc_sev_stationary, type = "response", what = "mu"))
Cyc_sev_sigma_stationary <- mean(predict(glm_Cyc_sev_stationary, type = "response", what = "sigma"))
Cyc_sev_stationary <- exp(Cyc_sev_mu_stationary + 0.5*Cyc_sev_sigma_stationary^2)
Cyc_loss_stationary  <- Cyc_sev_stationary*Cyc_freq_stationary
## Check: 
#TC_loss_pred_dat_yy/Cyc_loss_stationary

## Storms: --------
Storm_freq_stationary <- mean(predict(glm_Storm_freq_stationary, type ="response"))
Storm_sev_mu_stationary <- mean(predict(glm_Storm_sev_stationary, type = "response", what = "mu"))
Storm_sev_sigma_stationary <- mean(predict(glm_Storm_sev_stationary, type = "response", what = "sigma"))
Storm_sev_stationary <- exp(Storm_sev_mu_stationary + 0.5*Storm_sev_sigma_stationary^2)
Storm_loss_stationary  <- Storm_sev_stationary*Storm_freq_stationary
## Check: 
#Storm_loss_pred_dat_yy/Storm_loss_stationary

## ECL: --------------

ECL_freq_stationary <- mean(predict(glm_ECL_freq_stationary, type ="response"))
ECL_sev_mu_stationary <- mean(predict(glm_ECL_sev_stationary, type = "response", what = "mu"))
ECL_sev_sigma_stationary <- mean(predict(glm_ECL_sev_stationary, type = "response", what = "sigma"))
ECL_sev_stationary <- exp(ECL_sev_mu_stationary + 0.5*ECL_sev_sigma_stationary^2)
ECL_loss_stationary  <- ECL_sev_stationary*ECL_freq_stationary
## Check: 
#ECL_loss_pred_dat_yy/ECL_loss_stationary


## Hailstorm:  --------------

HS_freq_stationary <- mean(predict(glm_HS_freq_stationary, type ="response"))
HS_sev_mu_stationary <- mean(predict(glm_HS_sev_stationary, type = "response", what = "mu"))
HS_sev_sigma_stationary <- mean(predict(glm_HS_sev_stationary, type = "response", what = "sigma"))
HS_sev_stationary <- exp(HS_sev_mu_stationary + 0.5*HS_sev_sigma_stationary^2)
HS_loss_stationary  <- HS_sev_stationary*HS_freq_stationary
## Check: 
#HS_loss_pred_dat_yy/HS_loss_stationary

## Calculate total risk premium:  --------------
Hazard_loss_pred_stationary <- Flood_loss_stationary + Bush_loss_stationary + Cyc_loss_stationary + 
    Storm_loss_stationary + ECL_loss_stationary + HS_loss_stationary
# Check: 
#Hazard_loss_pred_dat/Hazard_loss_pred_stationary
```

```{r Caclulate the hazard variance under the stationary setting}
##Flood:
Flood_var_stationary <- Flood_freq_stationary*exp(2*Flood_sev_mu_stationary + Flood_sev_sigma_stationary^2)*exp(Flood_sev_sigma_stationary^2)
##Bush:
Bush_var_stationary <- Bush_freq_stationary*exp(2*Bush_sev_mu_stationary + Bush_sev_sigma_stationary^2)*exp(Bush_sev_sigma_stationary^2)
##Cyc:
Cyc_var_stationary <- Cyc_freq_stationary*exp(2*Cyc_sev_mu_stationary + Cyc_sev_sigma_stationary^2)*exp(Cyc_sev_sigma_stationary^2)
##Storm:
Storm_var_stationary <- Storm_freq_stationary*exp(2*Storm_sev_mu_stationary + Storm_sev_sigma_stationary^2)*exp(Storm_sev_sigma_stationary^2)
##ECL:
ECL_var_stationary <- ECL_freq_stationary*exp(2*ECL_sev_mu_stationary + ECL_sev_sigma_stationary^2)*exp(ECL_sev_sigma_stationary^2)
##HS:
HS_var_stationary <- HS_freq_stationary*exp(2*HS_sev_mu_stationary + HS_sev_sigma_stationary^2)*exp(HS_sev_sigma_stationary^2)

## Calculate the variance of total hazards: -------------

Hazard_var_agg_stationary <- Flood_var_stationary + Bush_var_stationary + Cyc_var_stationary + Storm_var_stationary + ECL_var_stationary + HS_var_stationary

## Check calculation of hazard variance: 
#Hazard_var_agg_mat/Hazard_var_agg_stationary
```

```{r Derive the premiums under the stationary case}
gross_premium_CAT_norm_stationary <- Hazard_loss_pred_stationary + risk_aversion_default*sqrt(Hazard_var_agg_stationary)

gross_premium_CAT_nominal_stationary <- gross_premium_CAT_norm_stationary * CPI_expected_stationary[2:(forecast_horizon+1)]/CPI_start *GDP_forecasts_stationary/Real_GDP_start

gross_premium_nominal_stationary <- gross_premium_CAT_nominal_stationary + E_nonCAT * (1 + CoV_nonCAT*risk_aversion_default) * Exposure_forecasts_stationary * (CPI_expected_stationary[2:(forecast_horizon+1)]/CPI_start)

## Check: 
#gross_premium_CAT_norm_mat/gross_premium_CAT_norm_stationary
#gross_premium_nominal_mat / gross_premium_nominal_stationary
```


#### Reinsurance premiums (stationary)

```{r Derive the weights allocated to different hazards under the stationary case}
## Calculate aggregate frequency:----------
aggr_freq_stationary <- Flood_freq_stationary + Bush_freq_stationary + Cyc_freq_stationary + 
    Storm_freq_stationary + ECL_freq_stationary + HS_freq_stationary

## Derive weights based on aggregate frequency: --------------
Flood_weights_stationary <- Flood_freq_stationary/aggr_freq_stationary
Bush_weights_stationary <- Bush_freq_stationary/aggr_freq_stationary
Cyc_weights_stationary <- Cyc_freq_stationary/aggr_freq_stationary
Storm_weights_stationary <- Storm_freq_stationary/aggr_freq_stationary
ECL_weights_stationary <- ECL_freq_stationary/aggr_freq_stationary
HS_weights_stationary <- HS_freq_stationary/aggr_freq_stationary
## Total weights: ----------
weights_freq_stationary_list <- c(Flood_weights_stationary, Bush_weights_stationary, Cyc_weights_stationary, Storm_weights_stationary, ECL_weights_stationary, HS_weights_stationary)
## Store severity distribution parameters: --------
mu_sev_stationary_list <- c(Flood_sev_mu_stationary, Bush_sev_mu_stationary, Cyc_sev_mu_stationary, Storm_sev_mu_stationary, ECL_sev_mu_stationary, HS_sev_mu_stationary)
sigma_sev_stationary_list <- c(Flood_sev_sigma_stationary, Bush_sev_sigma_stationary, Cyc_sev_sigma_stationary, Storm_sev_sigma_stationary, ECL_sev_sigma_stationary, HS_sev_sigma_stationary)
## Print out the parameters:
data.frame(Hazards = c("Flood", "Bushfire", "Cyclones", "Storms", "East Coast Low", "Hail"), freq = weights_freq_stationary_list * aggr_freq_stationary, mu = mu_sev_stationary_list , sigma = sigma_sev_stationary_list)
```



```{r Write a function to calculate reinsurance premium under the stationary case}
### Calculate aggregate distribution: -------------

calc_aggr_dist_stationary <- function(excess, limit_re, length.discr = 1000, market_cap = 1, disc_method = "rounding", mu_list = mu_sev_stationary_list, sigma_list = sigma_sev_stationary_list, weights_freq_list = weights_freq_stationary_list, freq_total = aggr_freq_stationary){
    ## Define other parameters:
    limit <- excess + limit_re
    stp <- ceiling(limit/length.discr)
    loss_seq <- seq(from = 0, to = limit, by = stp)
    ...
    ##########  Derive the discretised CDF:########## 
    fx_aggr_yy <- mixLN.discr.func(x, mu = mu_list + log(market_cap), sigma = sigma_list, weights = weights_freq_list, limit = limit, length = length.discr, method = disc_method)
    
    ########### Derive the aggregate distribution ########### 
    Fs_aggr_yy <- aggregateDist(method = "recursive", model.freq = "poisson",
      lambda = freq_total, model.sev = fx_aggr_yy, x.scale = stp, maxit = length.discr)
    fs_aggr_yy <- diff(Fs_aggr_yy)
    ########## Derive the truncated likelihood ########### 
    Fs_u <- cumsum(fs_aggr_yy)[u_index]
    Fs_l <- cumsum(fs_aggr_yy)[l_index]
...
calc_re_premium_stationary <- function(excess_list, limit_re_list, market_cap = 1){
      aggr_dist <- calc_aggr_dist_stationary(excess = excess_list, limit_re = limit_re_list, market_cap = market_cap, length.discr = 1000, disc_method = "rounding")
        # Calculated the expected reinsurance loss:
        E_Re_aggr <- aggr_dist$Es_aggr_trunc
        Var_Re_aggr <-aggr_dist$Var_aggr_trunc
return(list(Es_aggr_trunc = E_Re_aggr, Var_aggr_trunc = Var_Re_aggr))
}
```
...
```{r Calculate the reinsurance premiums under the stationary case}

re_premium_calcs_L_stationary <- calc_re_premium_stationary(excess_list = excess_default_Large, limit_re_list = limit_default_large, market_cap = market_share_input[market_share_input$Company == "Large", ]$Market_shares)
    
re_premium_calcs_M_stationary <- calc_re_premium_stationary(excess_list = excess_default_Medium, limit_re_list = limit_default_Medium, market_cap = market_share_input[market_share_input$Company == "Medium", ]$Market_shares)
    
re_premium_calcs_S_stationary <- calc_re_premium_stationary(excess_list = excess_default_Small, limit_re_list = limit_default_Small, market_cap = market_share_input[market_share_input$Company == "Small", ]$Market_shares)
    
    ### Calculate the total reinsurance premiums: 
    #### Note: we cap the reinsurance at the limit level; this is because if reinsurance premiums are greater than the limit, the insurers will have no incentive to purchase reinsurance. 
    re_premium_L_stationary <- pmin(re_premium_calcs_L_stationary$Es_aggr_trunc + risk_aversion_default * sqrt(re_premium_calcs_L_stationary$Var_aggr_trunc), limit_default_large)
    re_premium_M_stationary <- pmin(re_premium_calcs_M_stationary$Es_aggr_trunc + risk_aversion_default * sqrt(re_premium_calcs_M_stationary$Var_aggr_trunc), limit_default_Medium)
    re_premium_S_stationary <- pmin(re_premium_calcs_S_stationary$Es_aggr_trunc + risk_aversion_default * sqrt(re_premium_calcs_S_stationary$Var_aggr_trunc), limit_default_Small)
    
    re_premium_L_stationary_base <- re_premium_calcs_L_stationary$Es_aggr_trunc
    re_premium_M_stationary_base <- re_premium_calcs_M_stationary$Es_aggr_trunc 
    re_premium_S_stationary_base <- re_premium_calcs_S_stationary$Es_aggr_trunc
    
    ### Derive the total market reinsurance premium:
    reinsurance_premium_market_stationary <- re_premium_L_stationary*market_share_input[market_share_input$Company == "Large", ]$Numbers + re_premium_M_stationary*market_share_input[market_share_input$Company == "Medium", ]$Numbers + re_premium_S_stationary * market_share_input[market_share_input$Company == "Small", ]$Numbers
    
    reinsurance_premium_market_stationary_base <- re_premium_L_stationary_base*market_share_input[market_share_input$Company == "Large", ]$Numbers + re_premium_M_stationary_base*market_share_input[market_share_input$Company == "Medium", ]$Numbers + re_premium_S_stationary_base * market_share_input[market_share_input$Company == "Small", ]$Numbers

### Check reinsurance premium calculations: 
reinsurance_premium_multipliers <- reinsurance_premium_market_all_scenarios/reinsurance_premium_market_stationary
plot(x = 2023:(2023 + forecast_horizon - 1), y = reinsurance_premium_multipliers[, 1], type = "l", col = col_list[1], lwd = 2, ylim = c(1, 7), xlab = "Years", ylab = "Multipliers", main = "Ratios of reinsurance premiums projected to stationary case")
for(s in 2:4){
    points(x = 2023:(2023 + forecast_horizon - 1), y = reinsurance_premium_multipliers[, s], type = "l", col = col_list[s], lwd = 2)
}
legend("topleft", legend = scenario_list, col = col_list, lty = 1, lwd = 2)
```


```{r}
re_premium_calcs_L_stationary$Es_aggr_trunc
sqrt(re_premium_calcs_L_stationary$Var_aggr_trunc)
risk_aversion_default * sqrt(re_premium_calcs_L_stationary$Var_aggr_trunc)
```
...
### Surplus module  (stationary)

```{r Write a function to simulate surplus under the stationary case}
calc_surplus_func_full_cycle_stationary <- function(Risk_free_weight, 
                              market_cap,
                              reinsurance_excess,
                              reinsurance_limit_multiple, 
                              rates_constrain = FALSE,
                              ...
                              Var_CAT_loss, 
                              risk_aversion, 
                              CPI_pred, CPI_start = CPI_start, 
                              GDP_pred, GDP_start = Real_GDP_start, 
                              Exposure_start_market = Exposure_start, 
                              mu_equity = mu_TR_stationary, 
                              sigma_equity = sigma_TR_stationary,
                              Exposure_pred,
                              simulation_DFA_list_all, 
                              n.sims, 
                              horizon,
     ...
     ## Obtain the CPI and GDP corresponding to each scenario:
     CPI_pred <- CPI_pred[1:horizon]
     GDP_pred <- GDP_pred[1:horizon]
     Exposure_pred <- Exposure_pred[1:horizon]*market_cap
     Exposure_pred_other <- Exposure_pred_other_full[1:horizon]*market_cap
     Exposure_start <- Exposure_start_market * market_cap
    
     ## Calculate the gross premium:
     Premium_loading <- (sqrt(Var_CAT_loss[1:horizon])/Expected_CAT_Loss[1:horizon])*risk_aversion
     Var_nonCAT <- dispersion_nonCAT*Expected_Non_CAT_Loss^Power_Non_CAT_loss
     CoV_nonCAT <- sqrt(Var_nonCAT)/Expected_Non_CAT_Loss
     Premium_nonCAT_loading <- CoV_nonCAT*risk_aversion
     ...
     Premium_gross_CAT_norm_raw <- (1 +  Premium_loading_adjusted)*(Expected_CAT_Loss[1:horizon])
     ...
     Limit <- reinsurance_excess*reinsurance_limit_multiple
     ### Calculate the reinsurance premium using Panjer's recursion: 
     Re_loss_CAT <- calc_re_premium_stationary(excess_list = reinsurance_excess, limit_re_list = reinsurance_excess*reinsurance_limit_multiple, market_cap = market_cap)
     ...
     Re_premium_CAT <- Re_loss_CAT$Es_aggr_trunc * (1 + Re_premium_loading_adj) *(CPI_pred/CPI_start) * (GDP_pred/GDP_start)
     Re_premium_CAT_norm <- Re_loss_CAT$Es_aggr_trunc * (1 + Re_premium_loading_adj)
     simulation_DFA_list <- simulation_DFA_list_all
     ## Define a empty matrices to store the capital level and underwriting loss: 
     Capital_sims <- matrix(NA, nrow = length(CPI_pred), ncol = n.sims)
     Re_Capital_sims <- matrix(NA, nrow = length(CPI_pred), ncol = n.sims)
     #Capital_sims_normalised <- matrix(NA, nrow = length(CPI_pred), ncol = n.sims)
     UL_results <- c()
     Premium_net_sims <- matrix(NA, nrow = length(CPI_pred), ncol = n.sims)
     Premium_re_sims <- matrix(NA, nrow = length(CPI_pred), ncol = n.sims)
     sol_ratios_sims <- matrix(NA, nrow = length(CPI_pred) + 1, ncol = n.sims)
     capital_loading_sims <-  matrix(NA, nrow = length(CPI_pred) + 1, ncol = n.sims)
     ...
     ################ Calculate the investment return ################ ################
     ## Get the norminal risk-free rates:
     Short_rates_nominal <-  simulation_DFA_list$MEV_module$Nominal_rates[1:horizon, i]
     Short_rates_real <- simulation_DFA_list$MEV_module$Real_rates[1:horizon, i]
     
     ## Get the excess equity return:
     equity_return_excess <- rnorm(horizon, mean = mu_equity, sd = sigma_equity)
     #### Get the nominal equity return: 
     equity_return <- pmax(equity_return_excess + Short_rates_nominal, -1)
     equity_return_real <- pmax(equity_return_excess + Short_rates_real, -1)
     ## Get the total return
     ...
     for(k in 2:(length(Premium_gross_CAT)+1)){
         ...
         ## Assign the reinsurance premium: 
         re_premium_projections[k] <- P_re_sim
         ## Assign the solvency ratio:
         sol_ratios[k] <- S_prev_Re
         capital_loading[k] <- P_re_sim/Re_premium_CAT[k -1] - 1
     }
     Capital_sims[, i] <- capital_projections[2:(length(Premium_gross_CAT)+1)]
     UL_results[i] <- claims_cost_total[1]/(1 + investment_return[1]) - Premium_net[1]
     Premium_net_sims[, i] <- Premium_net[2:(length(Premium_gross_CAT)+1)]
     Premium_re_sims[, i] <- re_premium_projections[2:(length(Premium_gross_CAT)+1)]
     Re_Capital_sims[, i] <- re_capital_projections[2:(length(Premium_gross_CAT)+1)]
     sol_ratios_sims[, i] <- sol_ratios
     
     Claims_cost_list[i] <- claims_cost_total[1]
     Investment_returns_list[i] <- investment_return[1]
     capital_loading_sims[, i] <- capital_loading
     
    }
return(list(Capital_sims = Capital_sims, Premiums = Premium_net_sims, Premiums_Re = Premium_re_sims, Capital_sims_re = Re_Capital_sims, Premiums_Re_Unadj = Re_premium_CAT, Underwriting_Loss_sims = UL_results, Sol_ratios = sol_ratios_sims, capital_loading = capital_loading_sims))
}
```
...
<!-- ```{r Derive initial capital level under the stationary case, warning=FALSE} -->
<!--     ################################################# -->
<!--     ## Initial capital requirement for large size: ------------------------------- -->
<!--     ################################################# -->
<!--     UL_sims_Large_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default,  -->
<!--                                   market_cap = market_share_input[market_share_input$Company == "Large", ]$Market_shares, -->
<!--                                   reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Excess, -->
<!--                                   reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Excess,  -->
<!--                                   S_init = 0,  -->
...
<!--                                   Exposure_pred_other_full = Exposure_forecasts_other,  -->
<!--                                   Power_Non_CAT_loss = power_tw,  -->
<!--                                   Expected_CAT_Loss = rep(Hazard_loss_pred_stationary, forecast_horizon), -->
<!--                                   Var_CAT_loss = rep(Hazard_var_agg_stationary, forecast_horizon),  -->
<!--                                   risk_aversion = risk_aversion_default,  -->
<!--                                   CPI_pred = CPI_expected_stationary, CPI_start = CPI_start,  -->
<!--                                   GDP_pred = GDP_forecasts_stationary, GDP_start = Real_GDP_start,  -->
<!--                                   Exposure_pred = Exposure_forecasts_stationary, -->
<!--                                   Exposure_start = Exposure_start, -->
<!--                                   simulation_DFA_list_all = DFA_components_stationary, -->
<!--                                   n.sims = n.sims,  -->
<!--                                   horizon = 2, -->
<!--                                   assumption_alternative = FALSE) -->
...
<!--   UL_sims_Medium_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default,  -->
<!--                                   market_cap = market_share_input[market_share_input$Company == "Medium", ]$Market_shares, -->
<!--                                   reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Excess, -->
<!--                                   reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Excess,  -->
<!--                                   S_init = 0,  -->
...
<!--     ## Initial capital requirement for Small size: ------------------------------- -->
<!--     ################################################# -->
<!--      UL_sims_Small_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default,  -->
<!--                                   market_cap = market_share_input[market_share_input$Company == "Small", ]$Market_shares, -->
<!--                                   reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Excess, -->
<!--                                   reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Excess,  -->
<!--                                   S_init = 0,  -->
...
<!--                                   assumption_alternative = FALSE) -->
<!-- ## Calculate the initial capital requirements at the 99.5th quantile times the PCA multiple: :  -->
<!-- init_capital_L_stationary <- quantile(UL_sims_Large_stationary$Underwriting_Loss_sims, 0.995) * PCA_multiple_mean -->
<!-- init_capital_M_stationary <- quantile(UL_sims_Medium_stationary$Underwriting_Loss_sims, 0.995) * PCA_multiple_mean -->
<!-- init_capital_S_stationary <- quantile(UL_sims_Small_stationary$Underwriting_Loss_sims, 0.995) * PCA_multiple_mean -->
<!-- ``` -->

```{r Set initial capital to be the average of starting capital in the non-stationary case}
init_capital_L_stationary <- mean(init_capital_L)
init_capital_M_stationary <- mean(init_capital_M)
init_capital_S_stationary <- mean(init_capital_S)
```
...
```{r Plot of intial capital levels with excess and limit considered}
## Plot nominal capital levels:
hist(UL_sims_Large_stationary$Underwriting_Loss_sims/1e9, breaks = 100, xlab = "Underwriting loss ($Bn)", main = "Underwriting loss (first year) and initial capital (Stationary): Large")
abline(v = init_capital_L_stationary/1e9, lty = 2, col = "red")

hist(UL_sims_Medium_stationary$Underwriting_Loss_sims/1e9, breaks = 100, xlab = "Underwriting loss ($Bn)", main = "Underwriting loss (first year) and initial capital (Stationary): Medium")
abline(v = init_capital_M_stationary/1e9, lty = 2, col = "red")

hist(UL_sims_Small_stationary$Underwriting_Loss_sims/1e9, breaks = 100, xlab = "Underwriting loss ($Bn)", main = "Underwriting loss (first year) and initial capital (Stationary): Small")
abline(v = init_capital_S_stationary/1e9, lty = 2, col = "red")

## Compare with initial capital level under climate change scenarios: 
par(mfrow = c(1, 3))
## Plot nominal capital levels:
barplot(c(init_capital_L_stationary, init_capital_L)/1e9, names.arg = c("Stationary", scenario_list), main = "Initial capital (Large)", ylab = "Capital (Billions)")
barplot(c(init_capital_M_stationary, init_capital_M)/1e9, names.arg = c("Stationary", scenario_list), main = "Initial capital (Medium)", ylab = "Capital (Billions)")
barplot(c(init_capital_S_stationary, init_capital_S)/1e9, names.arg = c("Stationary", scenario_list), main = "Initial capital (Small)", ylab = "Capital (Billions)")

```

```{r Derive the capital simulations for the whole forecasting period under the stationary case, warning=FALSE}
#################################################
    ## Initial capital requirement for large size: -------------------------------
    #################################################
    capitalPremiums_sims_list_L_cycle_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default, 
                                  market_cap = market_share_input[market_share_input$Company == "Large", ]$Market_shares,
                                  reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Excess,
                                  reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Large", ]$Aggregate_Excess, 
                                  S_init = init_capital_L_stationary, 
                                  S_init_norm = init_capital_L_stationary,
                                  Expected_Non_CAT_Loss = mean(predict(model_nonCAT, type = "response")),
                                  dispersion_nonCAT = dispersion_nonCAT,
                                  Expected_Non_CAT_Loss_other = mean(predict(model_nonCAT_other, type = "response")), 
                                  dispersion_nonCAT_other = as.numeric(summary(model_nonCAT_other)$dispersion),
  ...
  capitalPremiums_sims_list_M_cycle_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default, 
                                  market_cap = market_share_input[market_share_input$Company == "Medium", ]$Market_shares,
                                  reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Excess,
                                  reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Medium", ]$Aggregate_Excess, 
                                  S_init = init_capital_M_stationary, 
                                  S_init_norm = init_capital_M_stationary,
                                  Expected_Non_CAT_Loss = mean(predict(model_nonCAT, type = "response")),
                                  dispersion_nonCAT = dispersion_nonCAT,
                                  Expected_Non_CAT_Loss_other = mean(predict(model_nonCAT_other, type = "response")), 
                                  dispersion_nonCAT_other = as.numeric(summary(model_nonCAT_other)$dispersion),
    ...
    ## Initial capital requirement for Small size: -------------------------------
    #################################################
     capitalPremiums_sims_list_S_cycle_stationary <- calc_surplus_func_full_cycle_stationary(Risk_free_weight = Risk_free_weight_default, 
                                  market_cap = market_share_input[market_share_input$Company == "Small", ]$Market_shares,
                                  reinsurance_excess = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Excess,
                                  reinsurance_limit_multiple = Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Limit/Reinsurance_structure_input[Reinsurance_structure_input$Size == "Small", ]$Aggregate_Excess, 
                                  S_init = init_capital_S_stationary, 
                                  S_init_norm = init_capital_S_stationary,
                                  Expected_Non_CAT_Loss = mean(predict(model_nonCAT, type = "response")),
                                  dispersion_nonCAT = dispersion_nonCAT,
                                  Expected_Non_CAT_Loss_other = mean(predict(model_nonCAT_other, type = "response")), 
                                  dispersion_nonCAT_other = as.numeric(summary(model_nonCAT_other)$dispersion),
...
```{r Calculate the capital results with reinsurance cycle considered under stationary case}
    ## Calculate the market surplus:
    capital_sims_list_Market_cycle_stationary <- capitalPremiums_sims_list_L_cycle_stationary$Capital_sims*market_share_input[market_share_input$Company == "Large", ]$Numbers + capitalPremiums_sims_list_M_cycle_stationary$Capital_sims*market_share_input[market_share_input$Company == "Medium", ]$Numbers + capitalPremiums_sims_list_S_cycle_stationary$Capital_sims*market_share_input[market_share_input$Company == "Small", ]$Numbers
    ## Calculate the reinsurance capital: 
    capital_re_sims_list_Market_cycle_stationary <- capitalPremiums_sims_list_L_cycle_stationary$Capital_sims_re*market_share_input[market_share_input$Company == "Large", ]$Numbers + capitalPremiums_sims_list_M_cycle_stationary$Capital_sims_re*market_share_input[market_share_input$Company == "Medium", ]$Numbers + capitalPremiums_sims_list_S_cycle_stationary$Capital_sims_re*market_share_input[market_share_input$Company == "Small", ]$Numbers
    ## Calculate the reinsurance premiums:
    premium_sims_re_list_Market_cycle_stationary <- capitalPremiums_sims_list_L_cycle_stationary$Premiums_Re*market_share_input[market_share_input$Company == "Large", ]$Numbers + capitalPremiums_sims_list_M_cycle_stationary$Premiums_Re*market_share_input[market_share_input$Company == "Medium", ]$Numbers + capitalPremiums_sims_list_S_cycle_stationary$Premiums_Re*market_share_input[market_share_input$Company == "Small", ]$Numbers
    ## Calculate the base reinsurance premiums:
    premium_re_stationary_base_nominal <-  capitalPremiums_sims_list_L_cycle_stationary$Premiums_Re_Unadj*market_share_input[market_share_input$Company == "Large", ]$Numbers + capitalPremiums_sims_list_M_cycle_stationary$Premiums_Re_Unadj*market_share_input[market_share_input$Company == "Medium", ]$Numbers + capitalPremiums_sims_list_S_cycle_stationary$Premiums_Re_Unadj*market_share_input[market_share_input$Company == "Small", ]$Numbers
    premium_re_stationary_base_norm <- premium_re_stationary_base_nominal
```


```{r}
    premium_re_stationary_base_nominal * CPI_start/CPI_pred * Real_GDP_start/Real_GDP_pred
```
...
```{r Adjust the capitals by setting the equity level after default to be zero (with reinsurance cyle considered)}
#capital_sims_list_Market_cycle_stationary
capital_sims_list_Market_adj_cycle_stationary <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    for(i in 1:n.sims){
        capital_sim <- capital_sims_list_Market_cycle_stationary[, i]
        ## Find the index of the first time that the surplus turns to negative: 
        first_negative_idx <- which(capital_sim <= 0)[1] 
        ## Set all numbers after the first negative number to zero
        if (!is.na(first_negative_idx) && first_negative_idx < length(capital_sim)) {
    ...
    capital_sims_list_Market_adj_cycle_stationary[, i] <- capital_sim
    }

```


Calculate the reinsurance market premium under the stationary setting: 


```{r Calculate reinsurance market premium under the stationary case}
## Define an empty matrix: --------
premium_sims_re_list_Market_norm_cycle_stationary <- matrix(NA, nrow = nrow(premium_sims_re_list_Market_cycle_stationary), ncol = ncol(premium_sims_re_list_Market_cycle_stationary))
#premium_sims_re_list_L_norm_cycle_stationary <- matrix(NA, nrow = nrow(premium_sims_re_list_Market_cycle_stationary), ncol = ncol(premium_sims_re_list_Market_cycle_stationary))
## Adjust the reinsurnace premiums: ---------
premium_sims_re_list_Market_norm_cycle_stationary <- premium_sims_re_list_Market_cycle_stationary * CPI_start/CPI_pred * Real_GDP_start/GDP_forecasts_stationary
```



Calculate the summary statistics for the adjusted primary insurers' market capitals under the stationary setting: 

```{r Calculate the market statistics for the adjusted capital levels (with reinsurance cycle considered)}
    ## Derive quantiles for market capital: 
    market_capital_adj_quantiles_list_cycle_stationary <-  apply(capital_sims_list_Market_adj_cycle_stationary, MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.005, quantile_high = 0.995))
    ## Derive median for market capital: 
    market_capital_adj_median_cycle_stationary <- apply(capital_sims_list_Market_adj_cycle_stationary , MARGIN = 1, FUN = median)
    ## Derive PD for market capital: 
    market_capital_adj_default_cycle_stationary <- apply(capital_sims_list_Market_adj_cycle_stationary , MARGIN = 1, FUN = function(x) mean(x <= 0))

```


Calculate the summary statistics for the adjusted re-insurers' market premiums under the stationary setting:

```{r Derive the reinsurance capital and premium statistics under stationary setting}
## Define the lower abnd upper bounds of stats calculated:
re_stats_lb <- 0.005
re_stats_ub <- 0.995
## Calculate the statistics:
    ## Normalised:
premium_re_quantiles_list_cycle_stationary_norm <- apply(premium_sims_re_list_Market_norm_cycle_stationary, MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = re_stats_lb, quantile_high = re_stats_ub))
    ## CPI-adjusted:
 #premium_re_quantiles_list_cycle_stationary_CPI_adj <- apply(premium_re_sims_list_Market_adj_all_scenarios_CPI_adj_cycle_stationary, MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = re_stats_lb, quantile_high = re_stats_ub))

```


```{r Calculate mean reinsurance premium under the stationary setting}
Stationary_solvent_indicator <- capital_re_sims_list_Market_cycle_stationary > 0
## Derive premium uplift under the solvent cases: 
Premium_re_sims_stationary_solvent <- premium_sims_re_list_Market_cycle_stationary/Stationary_solvent_indicator 
Premium_re_sims_stationary_solvent[is.infinite(Premium_re_sims_stationary_solvent) | is.nan(Premium_re_sims_stationary_solvent)] <- NA
Premium_re_stationary_solvent_mean <- apply(Premium_re_sims_stationary_solvent, MARGIN = 1, FUN = function(x) mean(x, na.rm = T))
```




<!-- #### Investigation of abnormal stationary results:  -->

<!-- Check the accuracy of solvency loading calculation by comparing with the simulated premium loading with the simulated solvency ratio path:  -->

<!-- ```{r} -->
<!-- premium_re_loadings_stationary_mat_test <- premium_sims_re_list_L_norm_cycle_stationary/reinsurance_premium_market_stationary - 1 -->
<!-- max_loadings_stationary <- apply(premium_re_loadings_stationary_mat_test, MARGIN = 1, FUN = function(x) which.max(x)) -->
<!-- #max_loadings_stationary -->
<!-- premium_re_loadings_stationary_mat_test[, 847] -->
<!-- capitalPremiums_sims_list_L_cycle_stationary$Sol_ratios[, 847] -->

<!-- premium_S_func_alt(P_ref = 1, S_prev = 0, S_ref = 1, k1 = 0.667, floor_param = 1) -->
<!-- plot(apply(capitalPremiums_sims_list_L_cycle_stationary$capital_loading, MARGIN = 1, FUN = mean), type = "l") -->
<!-- plot(apply(capitalPremiums_sims_list_L_cycle_stationary$capital_loading, MARGIN = 1, FUN = max), type = "l") -->
<!-- View(capitalPremiums_sims_list_L_cycle_stationary$capital_loading) -->
<!-- # apply(premium_re_loadings_stationary_mat, MARGIN = 1, FUN = median) -->
<!-- # apply(premium_re_loadings_stationary_mat, MARGIN = 1, FUN = mean) -->
<!-- # apply(premium_re_loadings_stationary_mat, MARGIN = 1, FUN = max) -->
<!-- ``` -->



<!-- ```{r Calculate the solvency ratios} -->

<!-- apply(capitalPremiums_sims_list_L_cycle_stationary$Sol_ratios, MARGIN = 1, FUN = mean) -->

<!-- premium_S_func_alt(P_ref = 1, S_prev = apply(capitalPremiums_sims_list_L_cycle_stationary$Sol_ratios, MARGIN = 1, FUN = mean), S_ref = 1, k1 = 0.667, floor_param = 1) -->
...
<!-- ```{r Calculate mean premium statistics by excluding default case (stationary case)} -->
<!-- ## Derive solvent indicators forStationary:  -->
<!-- Stationary_solvent_indicator <- capitalPremiums_sims_list_L_cycle_stationary$Capital_sims_re > 0 -->
<!-- apply(premium_sims_re_list_Market_norm_cycle_stationary/reinsurance_premium_market_stationary - 1, MARGIN = 1, FUN = max) -->
<!-- ## Derive premium uplift under the solvent cases:  -->
<!-- Premium_re_sims_stationary_solvent <- premium_sims_re_list_Market_norm_cycle_stationary/Stationary_solvent_indicator  -->
<!-- Premium_re_sims_stationary_solvent[is.infinite(Premium_re_sims_stationary_solvent) | is.nan(Premium_re_sims_stationary_solvent)] <- NA -->
<!-- Premium_re_stationary_solvent_mean <- apply(Premium_re_sims_stationary_solvent, MARGIN = 1, FUN = function(x) mean(x, na.rm = T)) -->
<!-- plot(Premium_re_stationary_solvent_mean/reinsurance_premium_market_stationary - 1, type = "l") -->
<!-- ``` -->



<!-- Using the capital loading directly calcualted from the simulations yield consistent results:  -->

<!-- ```{r} -->
<!-- capitalPremiums_sims_list_L_cycle_stationary$capital_loading[, 847] -->
<!-- ``` -->

<!-- Similarly, using the nominal value of the premiums (and base premiums) yield sensible results:  -->

<!-- ```{r} -->
<!-- premium_sims_re_list_Market_cycle_stationary[, 847]/premium_re_stationary_base_nominal - 1 -->

<!-- apply(premium_sims_re_list_Market_cycle_stationary/premium_re_stationary_base_nominal - 1, FUN = mean, MARGIN = 1) -->
<!-- ``` -->
...
```{r Define colour for the stationary plot}
col_stat <- gray(0.5)
```
...
```{r Plot of simulation results for climate module}
#par(mfrow = c(2,2))
...
par(mfrow = c(3, 2))
##Near-surface temperature:
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
plot_quantiles_scenario(NS_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Near-surface temperature", y_range = c(21, 27), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.8, lty = c(1,1,1,1), lwd = rep(2, 4))
# ##Air temperature:
# plot_quantiles_scenario(Air_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Air temperature", main = "Atmospheric temperature", y_range = c(-36, -30), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
# legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
##SST:
plot_quantiles_scenario(SST_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "SST", y_range = c(24, 27), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
##SST gradients
plot_quantiles_scenario(SST_grad_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "SST gradients", y_range = c(-2.1, -0.5), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
#MSLP
plot_quantiles_scenario(MSLP_quantiles_list, forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "MSLP", y_range = c(101150, 101600), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
##FWI
plot_quantiles_scenario(mfwixx_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Extreme FWI", y_range = c(90, 140), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
##rx5day
plot_quantiles_scenario(rx5day_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Extreme precipitation", y_range = c(25, 160), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.38, lty = c(1,1,1,1))
#dev.off()
```
...
```{r Find the quantiles of simulated hazards under the stationary assumption}
    ## Calculate the quantiles for event count: 
    ## Flood: 
   Flood_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$Flood, MARGIN = 1, FUN = function(x) multi_func(x)) 
   ## Bushfire: 
   Bushfire_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$Bushfire, MARGIN = 1, FUN = function(x) multi_func(x)) 
   ## TC: 
   TC_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$TC, MARGIN = 1, FUN = function(x) multi_func(x)) 
   ## Storm: 
   Storm_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$Storm, MARGIN = 1, FUN = function(x) multi_func(x)) 
   ## ECL: 
   ECL_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$ECL, MARGIN = 1, FUN = function(x) multi_func(x)) 
   ## Hailstorm: 
   Hailstorm_loss_stationary_quantiles_list <- apply(DFA_components_stationary$Hazard_module_sev$Hailstorm, MARGIN = 1, FUN = function(x) multi_func(x)) 
...
```{r Plot of simulation results for hazard losses}
...
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
## Flood
plot_quantiles_scenario(Flood_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Flood losses (normalised)", y_range = c(0, 2.5e10), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), Flood_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), Flood_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), Flood_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(Flood_loss_stationary_quantiles_list[1, 1:horizon], rev(Flood_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
### Plot the legends --------------
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 0.8, lty = c(3,1,1,1,1), lwd = rep(2, 4))

## Bushfire
plot_quantiles_scenario(Bushfire_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Bushfires losses (normalised)", y_range = c(0, 3e9), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), Bushfire_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), Bushfire_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), Bushfire_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(Bushfire_loss_stationary_quantiles_list[1, 1:horizon], rev(Bushfire_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
## TC
plot_quantiles_scenario(TC_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Tropical cyclones losses (normalised)", y_range = c(0, 1.8e10), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), TC_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), TC_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), TC_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(TC_loss_stationary_quantiles_list[1, 1:horizon], rev(TC_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
## Storm
plot_quantiles_scenario(Storm_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Storm losses (normalised)", y_range = c(0, 1.8e9), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), Storm_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), Storm_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), Storm_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(Storm_loss_stationary_quantiles_list[1, 1:horizon], rev(Storm_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
## ECL
plot_quantiles_scenario(ECL_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "East Coast Lows losses (normalised)", y_range = c(0, 9e8), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), ECL_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), ECL_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), ECL_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(ECL_loss_stationary_quantiles_list[1, 1:horizon], rev(ECL_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
## Hailstorms
plot_quantiles_scenario(Hailstorm_loss_quantiles_list , forecast_periods = start_year:(start_year+horizon - 1), var_name = "Values", main = "Hailstorm losses (normalised)", y_range = c(0, 2.5e9), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F)
## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), Hailstorm_loss_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 2, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), Hailstorm_loss_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), Hailstorm_loss_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 1.5, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(Hailstorm_loss_stationary_quantiles_list[1, 1:horizon], rev(Hailstorm_loss_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
#legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
dev.off()
```
...
```{r Extended projection period for Bushfire loss}
#pdf("Plotting/Hazards/Simulations_Bushfire_losses_extended.pdf")
## Bushfire
plot_quantiles_scenario(Bushfire_loss_quantiles_list , forecast_periods = start_year:(start_year+78 - 1), var_name = "Values", main = "Bushfires losses (normalised)", y_range = c(0, 4.5e9), colour_list = col_list, horizon = 78, transparency = 0.1, label_percents = F)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.8, lty = c(1,1,1,1), lwd = rep(2, 4))
#dev.off()
```

```{r Calculate the projected increase in bushfire loss by 2100}
horizon_end <- 78

paste("Percentage increase in Bushfire loss in SSP 8.5 relative to its starting value (mean):", Bushfire_loss_quantiles_list[[4]][2, horizon_end]/Bushfire_loss_quantiles_list[[4]][2, 1] - 1)
paste("Percentage increase in Bushfire loss in SSP 8.5 relative to SSP 2.6 (mean):", Bushfire_loss_quantiles_list[[4]][2, horizon_end]/Bushfire_loss_quantiles_list[[1]][2, horizon_end] - 1)
paste("Percentage increase in Bushfire loss in SSP 8.5 relative to its starting value (95th quantile):", Bushfire_loss_quantiles_list[[4]][3, horizon_end]/Bushfire_loss_quantiles_list[[4]][3, 1] - 1)
paste("Percentage increase in Bushfire loss in SSP 8.5 relative to SSP 2.6 (95th quantile):", Bushfire_loss_quantiles_list[[4]][3, horizon_end]/Bushfire_loss_quantiles_list[[1]][3, horizon_end] - 1)
```


```{r Investigation of the peaks in the flood loss simulations}
par(mfrow = c(2, 2))
horizon <- 38
#y_range_hazard_sev <- c(0,1.4e10)
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
# Plot the 99.5th quantile: 
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Hazards/Extreme_precipitation_SSP85_95quantile.pdf")
### Plot rx5day: 
plot(x = start_year:(start_year+horizon - 1), y = rx5day_quantiles_list[[4]][3, 1:horizon], lty = 2, lwd = 2, col = col_list[4], type = "l", xlab = "Forecast Period", main = "Extreme precipitation (rx5day)", ylab = "Values")
abline(v = start_year+ which.max(Flood_loss_quantiles_list[[4]][2, 1:horizon]) - 1, lty = 4, lwd = 2, col = "red")
legend("topleft", legend = c("95th quantile (SSP 8.5)"), lty = 2, lwd = 2, col = col_list[4], cex = 0.7)
#dev.off()
### Plot flood loss: 
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Hazards/Flood_loss_SSP85_95quantile.pdf")
plot(x = start_year:(start_year+horizon - 1), y = Flood_loss_quantiles_list[[4]][3, 1:horizon], lty = 2, lwd = 2, col = col_list[4], type = "l", ylim = c(min(Flood_loss_quantiles_list[[4]][2, 1:horizon]), max(Flood_loss_quantiles_list[[4]][3, 1:horizon])), xlab = "Forecast Period", main = "Flood loss", ylab = "Values")
abline(v = start_year+ which.max(Flood_loss_quantiles_list[[4]][2, 1:horizon]) - 1, lty = 4, lwd = 2, col = "red")
legend("topleft", legend = c("95th quantile (SSP 8.5)"), lty = 2, lwd = 2, col = col_list[4], cex = 0.7)
#dev.off()
...
# Plot the mean
### Plot rx5day: 
plot(x = start_year:(start_year+horizon - 1), y = rx5day_quantiles_list[[4]][2, 1:horizon], lty = 1, lwd = 2, col = col_list[4], type = "l", xlab = "Forecast Period", main = "Extreme precipitation (rx5day)", ylab = "Values")
abline(v = start_year+ which.max(Flood_loss_quantiles_list[[4]][2, 1:horizon]) - 1, lty = 4, lwd = 2, col = "red")
legend("topleft", legend = c("Mean (SSP 8.5)"), lty = 1, lwd = 2, col = col_list[4], cex = 0.7)
### Plot flood loss: 
plot(x = start_year:(start_year+horizon - 1), y = Flood_loss_quantiles_list[[4]][2, 1:horizon], lty = 1, lwd = 2, col = col_list[4], type = "l", xlab = "Forecast Period", main = "Flood loss", ylab = "Values")
abline(v = start_year+ which.max(Flood_loss_quantiles_list[[4]][2, 1:horizon]) - 1, lty = 4, lwd = 2, col = "red")
legend("topleft", legend = c("Mean (SSP 8.5)"), lty = 1, lwd = 2, col = col_list[4], cex = 0.7)
```
...
```{r Benchmarking of ECL freqeuncy}
delta_ECL_count_2060 <- c()
for(s in 1:4){
   delta_ECL_count_2060[s] <- ECL_count_quantiles_list[[s]][2, horizon]/ECL_count_quantiles_list[[s]][2, 1] - 1
}

delta_ECL_count_2040 <- c()
for(s in 1:4){
   delta_ECL_count_2040[s] <- ECL_count_quantiles_list[[s]][2, horizon - 20]/ECL_count_quantiles_list[[s]][2, 1] - 1
}

paste("Change in ECL frequency (SSP 8.5; 2060; 3 degree rise:)", delta_ECL_count_2060[4])
paste("Change in ECL frequency (SSP 8.5; 2040; 2 degree rise:)", delta_ECL_count_2040[4])
```
...
log_equity_returns_nominal_general_cum_list <- list()
log_equity_returns_nominal_brown_cum_list <- list()
log_equity_returns_nominal_cum_list <- list()
for(s in 1:4){
    ...
    log_equity_returns_nominal_general_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    log_equity_returns_nominal_brown_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    log_equity_returns_nominal_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    for(i in 1:n.sims){
         ...
         equity_return_nominal_general <- pmax(other_return + Short_rates_nominal, -1)
         equity_return_nominal_brown <- pmax(brown_return + Short_rates_nominal, -1)
         ## Get the total return
         ...
         ## Get the cumulative equity returns:
         log_equity_returns_nominal_general_cum_mat[, i] <- log(cumprod(1 + equity_return_nominal_general))
         log_equity_returns_nominal_brown_cum_mat[, i] <- log(cumprod(1 + equity_return_nominal_brown))
        
         ## Get the cumulative real and nominal investment returns:
         log_equity_returns_nominal_cum_mat[, i] <- log(cumprod(1 + equity_return_nominal))
...
log_equity_returns_nominal_cum_list[[s]] <- log_equity_returns_nominal_cum_mat  
log_equity_returns_nominal_general_cum_list[[s]] <- log_equity_returns_nominal_general_cum_mat
log_equity_returns_nominal_brown_cum_list[[s]] <- log_equity_returns_nominal_brown_cum_mat
...
gdp_potential_growth_mat <- matrix(NA, nrow = forecast_horizon, ncol = 4)
...
Adj_consumption_growth_quantiles_all_scenarios_list <- list()
for(s in 1:4){
    ...
    gdp_potential_growth_mat[, s] <- gdp_growth_base
    ...
    Adj_consumption_growth_quantiles_all_scenarios_list[[s]] <- apply(Adj_consumption_growth_list[[s]], MARGIN = 1, FUN = function(x) multi_func_withMed2(x[is.finite(x) == T], quantile_low = 0.05, quantile_high = 0.95))
...
log_equity_returns_nominal_general_quantiles_all_scenarios_list <- list()
log_equity_returns_nominal_brown_quantiles_all_scenarios_list <- list()
log_equity_returns_nominal_cum_quantiles_all_scenarios_list <- list()
for(s in 1:4){
    ...
    log_equity_returns_nominal_cum_quantiles_all_scenarios_list[[s]] <- apply(log_equity_returns_nominal_cum_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x[is.finite(x) == T], quantile_low = 0.05, quantile_high = 0.95))
    ...
    log_equity_returns_nominal_general_quantiles_all_scenarios_list[[s]] <- apply(log_equity_returns_nominal_general_cum_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
     log_equity_returns_nominal_brown_quantiles_all_scenarios_list[[s]] <- apply(log_equity_returns_nominal_brown_cum_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
...
```{r Calculate cumulative investment returns under the stationary case}
## Calculate the simulated cumulative investment returns: -------------
log_investment_returns_nominal_cum_stationary  <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    for(i in 1:n.sims){
         Short_rates_nominal <- DFA_components_stationary$MEV_module$Nominal_rates[, i]
         ## Get the excess equity return:
         equity_return_excess <- rnorm(forecast_horizon, mean = mu_TR_stationary, sd = sigma_TR_stationary)
         #### Get the nominal equity return: 
         equity_return_nominal <- pmax(equity_return_excess + Short_rates_nominal, -1)
         ## Get the total returns:
         investment_return_nominal <- Risk_free_weight_default*Short_rates_nominal + (1-Risk_free_weight_default)*equity_return_nominal
         ## Get the cumulative real and nominal investment returns:
         log_investment_returns_nominal_cum_stationary[, i] <- log(cumprod(1 + investment_return_nominal))
    }

## Calculate the quantile statistics for the cumulative investment returns: -------
log_investment_returns_nominal_cum_stationary_quantiles <- apply(log_investment_returns_nominal_cum_stationary,  MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
```
...
plot_quantiles_scenario(log_investment_returns_nominal_quantiles_all_scenarios_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Cumulative returns (log)", main = "Cumulative investment returns (log-scale)", y_range = c(-0.2, 5), colour_list = col_list, horizon = horizon , transparency = 0.1, label_percents = F, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, mean_lwd = 4, quantiles_lwd = 2)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 1, lty = c(3,1,1,1,1), lwd = rep(4, 4))

# Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), log_investment_returns_nominal_cum_stationary_quantiles[2, 1:horizon], col = col_stat, lwd = 4, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), log_investment_returns_nominal_cum_stationary_quantiles[1, 1:horizon], col = col_stat, lwd = 2, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), log_investment_returns_nominal_cum_stationary_quantiles[3, 1:horizon], col = col_stat, lwd = 2, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(log_investment_returns_nominal_cum_stationary_quantiles[1, 1:horizon], rev(log_investment_returns_nominal_cum_stationary_quantiles[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
#dev.off()
```
...
plot(x = start_year:(start_year + horizon - 1), y =  log_gdp_potential_cum_growth_mat[1:horizon, 1], col = col_list[1], type = "l", ylim = c(0, 1.5), xlab = "Years", ylab = "Compounded growth (log)", main = "Compounded economic growth (log)", cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4)
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y =  log_gdp_potential_cum_growth_mat[1:horizon, s], col = col_list[s], type = "l", lwd = 3)
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()
```
...
horizon_total <- 78
################################################################################
########## Plot the Log Economic Projections ###################################
################################################################################
plot(x = start_year:(start_year + horizon_total - 1), y =  log(GDP_projections_SSP_interpolate[-c(1:2), 2]), col = col_list[1], type = "l", lwd = 2, ylim = c(28.5, 31), xlab = "Years", ylab = "GDP (log-scale)", main = "Gross domestic product (log-scale) in Australia (in real terms)")
for(s in 2:3){
points(x = start_year:(start_year + horizon_total - 1), y = log(GDP_projections_SSP_interpolate[-c(1:2), s + 1]), col = col_list[s], type = "l", lwd = 2)
}
points(x = start_year:(start_year + horizon_total - 1), y = log(GDP_projections_SSP_interpolate[-c(1:2), 6]), col = col_list[4], type = "l", lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
################################################################################
########## Plot the Total Economic Projections ###################################
################################################################################
#pdf("Plotting/Asset/RealGDP_Australia.pdf")
plot(x = start_year:(start_year + horizon_total - 1), y =  (GDP_projections_SSP_interpolate[-c(1:2), 2]), col = col_list[1], type = "l", lwd = 2, ylim = c(0, max(GDP_projections_SSP_interpolate[-c(1:2), 6])), xlab = "Years", ylab = "GDP", main = "Gross domestic product in Australia (in real terms)")
for(s in 2:3){
points(x = start_year:(start_year + horizon_total - 1), y = (GDP_projections_SSP_interpolate[-c(1:2), s + 1]), col = col_list[s], type = "l", lwd = 2)
}
points(x = start_year:(start_year + horizon_total - 1), y = (GDP_projections_SSP_interpolate[-c(1:2), 6]), col = col_list[4], type = "l", lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 0.6, lty = c(1,1,1,1))
#dev.off()
```
...
Insure_damage_ratio_quantiles_list <- list()
for(s in 1:4){
    Insure_damage_ratio_quantiles_list[[s]] <- apply(DFA_components_all_scenarios[[s]]$Liabilities_module$CAT_damage_ratio, MARGIN = 1, FUN = function(x) multi_func(x))
...
plot_quantiles_scenario_2(Eco_damage_ratio_quantiles_list, forecast_periods = start_year:(start_year+horizon - 1), var_name = "Damage ratios", main = "Ratios of uninsured damage to GDP", y_range = c(0, 0.02), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = T, step_plot_perc = 0.005, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4, lwd_quantiles = 3)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()
## Total damage ratios:
#pdf("Plotting/Asset/Total_damage_ratios_2060.pdf")
plot_quantiles_scenario_2(Total_damage_ratio_quantiles_list, forecast_periods = start_year:(start_year+horizon - 1), var_name = "Damage ratios", main = "Ratios of total damage to GDP", y_range = c(0, 0.03), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = T, step_plot_perc = 0.005, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4, lwd_quantiles = 3)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()
```

Plot the cumulative investment returns for brown and the general portfolio: 

```{r Plot the cumulative equity returns for brown and general portfolios}
#pdf("Plotting/Asset/Simulations_cum_Brown_investment_returns_2060.pdf")
## Brown portfolios: 
plot_quantiles_scenario( log_equity_returns_nominal_general_quantiles_all_scenarios_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Cumulative returns (log)", main = "Cumulative equity returns (brown portfolio; log-scale)", y_range = c(-0.2, 7), colour_list = col_list, horizon = horizon , transparency = 0.1, label_percents = F, cex.main = 1, cex.lab = 1.5, cex.axis = 1.5, mean_lwd = 4, quantiles_lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()

#pdf("Plotting/Asset/Simulations_cum_General_investment_returns_2060.pdf")
## General portfolios
plot_quantiles_scenario( log_equity_returns_nominal_brown_quantiles_all_scenarios_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Cumulative returns (log)", main = "Cumulative investment returns (general portfolio; log-scale)", y_range = c(-0.2, 7), colour_list = col_list, horizon = horizon , transparency = 0.1, label_percents = F, cex.main = 1, cex.lab = 1.5, cex.axis = 1.5, mean_lwd = 4, quantiles_lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()
## Comparison of mean (cumulative): 
#plot(x = start_year:(start_year+ 78 - 1), y = log_equity_returns_nominal_general_quantiles_all_scenarios_list[[1]][2, ], lwd = 3, col = col_list[1], type = "l")
#points(x = start_year:(start_year+ 78 - 1), y =log_equity_returns_nominal_brown_quantiles_all_scenarios_list[[1]][2, ], lwd = 3, col = col_list[1], type = "l", lty = 2)
```

```{r Compare the mean ecess returns}
## Derive the mean excess returns:
equity_TR_excess_quantiles_all_scenarios_list <- list()
equity_TR_excess_brown_quantiles_all_scenarios_list <- list()
for(s in 1:4){
    equity_TR_excess_quantiles_all_scenarios_list[[s]] <- apply(equity_TR_excess_adj_OP_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x))
    equity_TR_excess_brown_quantiles_all_scenarios_list[[s]] <- apply(equity_TR_excess_adj_OP_brown_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x))
}
## Plot the mean excess return across scenarios:
plot(x = start_year:(start_year+ horizon - 1), y = equity_TR_excess_quantiles_all_scenarios_list[[1]][2, 1:horizon], type = "l", col = col_list[1], lty = 1, lwd = 2, ylim = c(0.05, 0.07), xlab = "Years", ylab = "Returns", main = "Equity returns: brown vs. general portfolios")
points(x = start_year:(start_year+ horizon - 1), y =equity_TR_excess_brown_quantiles_all_scenarios_list[[1]][2, 1:horizon], type = "l", col = col_list[1], lty = 2, lwd = 2)
for(s in 2:4){
    points(x = start_year:(start_year+ horizon - 1), y =equity_TR_excess_quantiles_all_scenarios_list[[s]][2, 1:horizon], type = "l", col = col_list[s], lty = 1, lwd = 2)
    points(x = start_year:(start_year+ horizon - 1), y =equity_TR_excess_brown_quantiles_all_scenarios_list[[s]][2, 1:horizon], type = "l", col = col_list[s], lty = 2, lwd = 2)
}
legend("topright", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
## Plot the relative difference in mean excess returns between general and brown portfolios: 
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Asset/Relative_returns_differrence_brownVsGeneral.pdf")
plot(x = start_year:(start_year+ horizon - 1), y = equity_TR_excess_brown_quantiles_all_scenarios_list[[1]][2, 1:horizon]/equity_TR_excess_quantiles_all_scenarios_list[[1]][2, 1:horizon] -1, type = "l", col = col_list[1], lty = 1, lwd = 3, ylim = c(-0.05, 0.05), xlab = "Years", ylab = "Relative difference", main = "Relative return difference: brown vs. general portfolios (%)", yaxt = "n")
for(s in 2:4){
    points(x = start_year:(start_year+ horizon - 1), y = equity_TR_excess_brown_quantiles_all_scenarios_list[[s]][2, 1:horizon]/equity_TR_excess_quantiles_all_scenarios_list[[s]][2, 1:horizon] -1, type = "l", col = col_list[s], lty = 1, lwd = 3)
}
axis(2, at = pretty(c(-0.05, 0, 0.05)), labels = paste0(pretty(c(-0.05, 0, 0.05)) * 100, "%"))
legend("topright", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list,  lty = c(1,1,1,1), lwd = rep(4, 4), cex = 0.9)
dev.off()
```

```{r Comparison of climate-adjusted economic growth and the potential growh}
## Compare the absolute values: 
plot(gdp_potential_growth_mat[2:(horizon+1), 1], type = "l", lwd = 3, lty = 1, col = col_list[1],  ylim = c(-0.01, 0.05))
points(Adj_consumption_growth_quantiles_all_scenarios_list[[1]][4, 2:(horizon+1)], type = "l", lwd = 3, lty = 2, col = col_list[1])
points(Adj_consumption_growth_quantiles_all_scenarios_list[[1]][1, 2:(horizon+1)], type = "l", lwd = 3, lty = 3, col = col_list[1])
for(s in 2:4){
    points(gdp_potential_growth_mat[2:(horizon+1), s], type = "l", lwd = 3, lty = 1, col = col_list[s])
points(Adj_consumption_growth_quantiles_all_scenarios_list[[s]][4, 2:(horizon+1)], type = "l", lwd = 3, lty = 2, col = col_list[s])
points(Adj_consumption_growth_quantiles_all_scenarios_list[[s]][1, 2:(horizon+1)], type = "l", lwd = 3, lty = 3, col = col_list[s])
}
## Smooth the relative percentage difference: 
smooth_adj_consumption_growth_mean_mat <- matrix(NA, nrow = horizon, ncol = 4)
smooth_adj_consumption_growth_5quantile_mat <- matrix(NA, nrow = horizon, ncol = 4)
for(s in 1:4){
    ## Smooth the mean: 
    adj_growth_mean <- Adj_consumption_growth_quantiles_all_scenarios_list[[s]][4, 2:(horizon+1)]/gdp_potential_growth_mat[2:(horizon+1), s] - 1
    fit_growth_mean <- smooth.spline(1:horizon, adj_growth_mean, spar = 0.9)
    ## Smooth the 5th quantile: 
    adj_growth_VaR5 <- Adj_consumption_growth_quantiles_all_scenarios_list[[s]][1, 2:(horizon+1)]/gdp_potential_growth_mat[2:(horizon+1), s] - 1
     fit_growth_VaR5 <- smooth.spline(1:horizon, adj_growth_VaR5, spar = 0.8)
    ## Store the results: 
    smooth_adj_consumption_growth_mean_mat[, s] <- predict(fit_growth_mean)$y
    smooth_adj_consumption_growth_5quantile_mat[, s] <- predict(fit_growth_VaR5)$y
}

## Plot the relative difference:
### Mean path: 
plot(smooth_adj_consumption_growth_mean_mat[, 1], type = "l", lwd = 3, lty = 1, col = col_list[1], ylim = c(-0.4,0.1))
for(s in 2:4){
    points(smooth_adj_consumption_growth_mean_mat[, s], type = "l", lwd = 3, lty = 1, col = col_list[s])
}
 points(smooth_adj_consumption_growth_5quantile_mat[, 4], type = "l", lwd = 3, lty = 2, col = col_list[4])
### 5th percentile:
# plot(smooth_adj_consumption_growth_5quantile_mat[, 1], type = "l", lwd = 3, lty = 1, col = col_list[1], ylim = c(-0.5,0.3))
# for(s in 2:4){
#     points(smooth_adj_consumption_growth_5quantile_mat[, s], type = "l", lwd = 3, lty = 1, col = col_list[s])
# }
```
...
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5", "Baseline (DICE-2023)", "Cost-benefit optimal (DICE-2023)", "2°C Target", "Baseline (DICE-2016)", "Cost-benefit optimal (DICE-2016)"), col = c(col_list, "#FF6347", "#FFB347", "#228B22","#FF6347", "#FFB347"), cex = 0.6, lty = c(1,1,1,1, 1, 1, 1, 2, 2), pch = c(rep(NA, 4), rep(16, 3), rep(17, 2)), lwd = c(rep(4, 4), rep(NA, 5)))
...
```{r Calculate the normalised gross premiums}
#par(mfrow = c(4, 1))
horizon <- 38

#pdf("Plotting/Premiums/Gross_CAT_premiums_normalised_2060.pdf")
## Plot the gross CAT premiums:
plot(x = start_year_proj:(start_year_proj + horizon - 1), y = gross_premium_CAT_norm_mat[1:horizon, 1], type = "l", col = col_list[1], xlab = "Years", ylab = "Premiums", main = "Gross CAT premiums (normalised)", ylim = c(3e9, 9e9), cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4)
for(s in 2:4){
    points(x = start_year_proj:(start_year_proj + horizon - 1), y = gross_premium_CAT_norm_mat[1:horizon, s], type = "l", col = col_list[s], lwd = 4)
}
abline(h = gross_premium_CAT_norm_stationary, col = col_stat, lwd = 4, lty = 3)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 1, lty = c(3,1,1,1,1), lwd = rep(4, 4))
#dev.off()
```
...
```{r Plot the historical proportions of CAT losses and projected proportions}
horizon <- 38
#pdf("Plotting/Premiums/Gross_CAT_premiums_proportions_2060_withHistorical.pdf")
plot(x = c(GI_Ultimate_Losses_PM$Year, start_year_proj:(start_year_proj + horizon - 1)), y = c(GI_Ultimate_Losses_PM$CAT_loss_annual/GI_Ultimate_Losses_PM$Ultimate_Cost, rep(NA, each = horizon)), type = "p", ylim = c(0, 0.4), pch = 16, xlab = "Years", ylab = "Proportions", main = "Projected proportion of CAT premiums", cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4)
for(s in 1:4){
    points(x = c(GI_Ultimate_Losses_PM$Year, start_year_proj:(start_year_proj + horizon - 1)), y = c(rep(NA, nrow(GI_Ultimate_Losses_PM)),prop_CAT_premium_total_mat[1:horizon, s]), type = "l", col = col_list[s], lwd = 4)
}
abline(h = mean(GI_Ultimate_Losses_PM$CAT_loss_annual/GI_Ultimate_Losses_PM$Ultimate_Cost), lty = 2, lwd = 2)
legend("topleft", legend = c("Historical observed proportion of CAT losses","Historical average proportion of CAT losses", "SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c("black", "black", col_list), cex = 0.78, lty = c(NA,2,1,1,1,1), pch = c(16, rep(NA, 5)), lwd = c(2, 2, rep(4, 4)))
#dev.off()
```
...
horizon <- 38
plot(x = start_year:(start_year+horizon - 1), y = premium_re_quantiles_list_cycle_norm[[1]][2, 1:horizon], type = "l", col = col_list[1], ylim = c(4.5e8, 2.5e9), xlab = "Years", ylab = "Reinsurance premium", main = "Reinsurance premiums (normalised; mean)", cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 4)
for(s in 2:4){
    points(x = start_year:(start_year+horizon - 1), y = premium_re_quantiles_list_cycle_norm[[s]][2, 1:horizon], type = "l", col = col_list[s], lty = 1, lwd = 4)
}

points(x = start_year:(start_year+horizon - 1), y = reinsurance_premium_market_stationary*Premium_re_stationary_solvent_mean[1:horizon]/premium_re_stationary_base_nominal[1:horizon], col = col_stat, type = "l", lwd = 4, lty = 3)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 0.8, lty =c(3, rep(1, 4)), lwd = rep(4, 4))
#dev.off()
```
...
## Mean:
plot(x = start_year:(start_year+horizon - 1), y = premium_re_quantiles_list_cycle_norm[[1]][2, 1:horizon]/premium_re_all_scanarios_base_norm_mat[1:horizon, 1] - 1, type = "l", col = col_list[1], ylim = c(0, 1.1), main = "Relative difference to base reinsurance premiums (mean)", xlab  = "Years", ylab = "Ratios", yaxt = "n", cex.main = 1.4, cex.lab = 1.5, cex.axis = 1.5, lwd = 4)
axis(2, at = seq(0, 1.1, by = 0.2), labels = paste0(seq(0, 1, by = 0.2) * 100, '%'), las = 1)
for(s in 2:4){
    points(x = start_year:(start_year+horizon - 1), y = premium_re_quantiles_list_cycle_norm[[s]][2, 1:horizon]/premium_re_all_scanarios_base_norm_mat[1:horizon, s] - 1, type = "l", col = col_list[s], lwd = 4)
}
points(x = start_year:(start_year+horizon - 1), y = Premium_re_stationary_solvent_mean[1:horizon]/premium_re_stationary_base_nominal[1:horizon] - 1, type = "l", col = col_stat, lwd = 4, lty = 3)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 1, lty = c(3, 1,1,1,1), lwd = rep(3, 4))
#dev.off()
```
...
```{r Calculate underwriting loss under the stationary case}
## Calculate reinsurance recoveries for each company under the stationary assumption:------
    re_calc_stationary_L <- calc_reinsurance_recoveries_norm_stationary(horizon = 78, market_cap = market_share_input[market_share_input$Company == "Large", ]$Market_shares, reinsurance_excess = excess_default_Large, reinsurance_limit_multiple = limit_default_large/excess_default_Large, DFA_list = DFA_components_stationary)

    re_calc_stationary_M <- calc_reinsurance_recoveries_norm_stationary(horizon = 78, market_cap = market_share_input[market_share_input$Company == "Medium", ]$Market_shares,  reinsurance_excess = excess_default_Medium, reinsurance_limit_multiple = limit_default_Medium/excess_default_Medium, DFA_list = DFA_components_stationary)
    
    re_calc_stationary_S <- calc_reinsurance_recoveries_norm_stationary(horizon = 78, market_cap = market_share_input[market_share_input$Company == "Small", ]$Market_shares,  reinsurance_excess = excess_default_Small, reinsurance_limit_multiple = limit_default_Small/excess_default_Small, DFA_list = DFA_components_stationary)
    
    ## Calculate reinsurance recoveries for the whole market: ------------------------
    CAT_net_losses_market_stationary_mat <- re_calc_stationary_L$CAT_loss_net * market_share_input[market_share_input$Company == "Large", ]$Numbers + re_calc_stationary_M$CAT_loss_net * market_share_input[market_share_input$Company == "Medium", ]$Numbers + re_calc_stationary_S$CAT_loss_net * market_share_input[market_share_input$Company == "Small", ]$Numbers

# ## Calculate underwriting loss under the stationary case: ---------------- 
    
UL_losses_market_stationary_mat <- CAT_net_losses_market_stationary_mat - c(gross_premium_CAT_norm_stationary - reinsurance_premium_market_stationary)

UL_losses_market_stationary_quantiles_list <- apply(UL_losses_market_stationary_mat, MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
```
...
horizon <- 38
plot_quantiles_scenario(UL_losses_market_quantiles_all_scenarios_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Underwriting losses", main = "Simulated underwriting losses (normalised)", y_range = c(-6e9, 2.3e10), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, mean_lwd = 4, quantiles_lwd = 3)

## Add quantiles for stationary case:----------------
        lines(start_year:(start_year+horizon - 1), UL_losses_market_stationary_quantiles_list[2, 1:horizon], col = col_stat, lwd = 4, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), UL_losses_market_stationary_quantiles_list[1, 1:horizon], col = col_stat, lwd = 3, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), UL_losses_market_stationary_quantiles_list[3, 1:horizon], col = col_stat, lwd = 3, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(UL_losses_market_stationary_quantiles_list[1, 1:horizon], rev(UL_losses_market_stationary_quantiles_list[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
...
```{r Plot of expected and median surplus with adjusted capital (with reinsuranc cycle considered)}
#par(mfrow = c(2, 1))
## Plot of median surplus (with reinsurance): 
#pdf("Plotting/Surplus/Median_market_surplus_re_cycle.pdf")
horizon <- 38
plot(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_median_cycle[1:horizon, 1]), col = col_list[1], type = "l", xlab = "Years", ylab = "Median surplus", main = "Median market surplus (log-scale)", ylim = c(23, 32), lwd = 4, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5)
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_median_cycle[1:horizon, s]), col = col_list[s], type = "l", lwd = 4)
}
## Add the stationary case: ----------------
points(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_median_cycle_stationary[1:horizon]), lwd = 4, type = "l", col = col_stat, lty = 3)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 1, lty = c(3, 1,1,1,1), lwd = rep(4, 4))
#dev.off()

## Plot of mean surplus (with reinsurance): 
#pdf("Plotting/Surplus/Mean_market_surplus_re_cycle.pdf")
plot(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_quantiles_list_cycle[[1]][2, 1:horizon]), col = col_list[1], type = "l", xlab = "Years", ylab = "Mean surplus", main = "Mean market surplus (log-scale)", ylim = c(23, 32), lwd = 4, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5)
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_quantiles_list_cycle[[s]][2, 1:horizon]), col = col_list[s], type = "l", , lwd = 4)
}
## Add the stationary case: ----------------
points(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_quantiles_list_cycle_stationary[2, 1:horizon]), lwd = 4, type = "l", lty = 3, col = col_stat)
...
horizon <- 38
par(mfrow = c(2, 2))
par(mar = c(2.48, 2.48, 2.48, 1), mgp = c(1.5, 0.5, 0))
for(s in 1:4){
...
#Plot:
plot(x = log_investment_returns_nominal_median_all_scenarios_mat[1:horizon, s], y = log(market_capital_adj_median_cycle[1:horizon, s]), col = col_list[s], pch = 16, xlab = "Median investment returns (log)", ylab = "Median surplus (log)", main =scenario_list[s], ylim = c(25, 31))
## add the fitted line: 
points(x = log_investment_returns_nominal_median_all_scenarios_mat[1:horizon, s], y = fitted(Inv_lm), type = "l", lty = 2, lwd = 3)
legend("bottomright", legend = paste("R-squared:", round(summary(Inv_lm)$r.squared, 3)), lty = 2, cex = 0.9, lwd = 2)
}
#dev.off()
...
#Plot:
plot(x = log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][2, 1:horizon], y = log(market_capital_adj_quantiles_list_cycle[[s]][2, 1:horizon]), col = col_list[s], pch = 16, xlab = "Mean investment returns (log)", ylab = "Expected surplus (log)", main = scenario_list[s], ylim = c(25, 31))
## add the fitted line: 
points(x = log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][2, 1:horizon], y = fitted(Inv_lm), type = "l", lwd = 3, lty = 2)
legend("bottomright", legend = paste("R-squared:", round(summary(Inv_lm)$r.squared, 3)), lty = 2, cex = 0.9, lwd = 2)
}
#dev.off()
```
...
horizon <- 38
plot(x = start_year:(start_year + horizon - 1), y = market_capital_adj_default_cycle[1:horizon, 1], col = col_list[1], type = "l", ylim = c(0, 0.3), xlab = "Years", ylab = "Insolvency probabilities", main = "Market insolvency probability", lwd = 4, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5)
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = market_capital_adj_default_cycle[1:horizon, s], col = col_list[s], type = "l", lwd = 4)
}
points(x = start_year:(start_year + horizon - 1), y = market_capital_adj_default_cycle_stationary[1:horizon], type = "l", lwd = 4, lty = 3, col = col_stat)
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 1, lty = c(3,1,1,1,1), lwd = rep(4, 5))
#dev.off()
```
...
```{r Calculte the LGD ratios for adjusted capital levels (with reinsurance cycle considered) under the stationary case}
## Create empty matries to store LGD ratios:
LGD_ratios_lower_quant_stationary <- c()
LGD_ratios_mean_stationary <- c()
LGD_ratios_upper_quant_stationary <- c()
 
for(i in 1:forecast_horizon){
        ## Calculate the dollar values of loss-at-insolvency: 
        Loss <- pmax(-capital_sims_list_Market_adj_cycle_stationary[i, ], 0)
        Loss_at_default <- Loss[which(Loss > 0)]
        ## Derive the total insurance claims liabilities payable to policyholders:
        Exposure_total_Liab <- DFA_components_stationary$Liabilities_module$Non_CAT_loss_nominal[i, ] + DFA_components_stationary$Liabilities_module$Non_CAT_other_loss_nominal[i, ] + CAT_net_losses_market_stationary_mat[i, ]
        ## Derive the Exposure At Default: 
        EAD <- Exposure_total_Liab[which(Loss > 0)]
        ## Calculate the average LGD ratios:
        LGD_ratios <- pmin(Loss_at_default/EAD, 1)
        ## Store the outputs for LGD ratios
        LGD_ratios_mean_stationary[i] <- mean(LGD_ratios)
        LGD_ratios_lower_quant_stationary[i] <- quantile(LGD_ratios, 0.05)
        LGD_ratios_upper_quant_stationary[i] <- quantile(LGD_ratios, 0.95)
        
}
LGD_quantiles_stationary_list_ratios_adj_cycle <- rbind(LGD_ratios_lower_quant_stationary, LGD_ratios_mean_stationary, LGD_ratios_upper_quant_stationary)
LGD_quantiles_stationary_list_ratios_adj_cycle[is.na(LGD_quantiles_stationary_list_ratios_adj_cycle)] <- 0
```
...
horizon <- 50
plot_quantiles_scenario(LGD_quantiles_all_scenarios_list_ratios_adj_cycle, forecast_periods = start_year:(start_year + horizon - 1), var_name = "Deficit given insolvency", main = "Market deficit given insolvency (ratios)", y_range = c(0, 1), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = F, cex.main.choice = 1.5, cex.lab.choice = 1.5, cex.axis.choice = 1.5, xlab.name = "Years", mean_lwd = 4, quantiles_lwd = 3)
## Plot the results under the stationary case: -------
lines(start_year:(start_year+horizon - 1), LGD_quantiles_stationary_list_ratios_adj_cycle[2, 1:horizon], col = col_stat, lwd = 4, lty = 3) # 50% quantile (median)
        lines(start_year:(start_year+horizon - 1), LGD_quantiles_stationary_list_ratios_adj_cycle[1, 1:horizon], col = col_stat, lwd = 3, lty = 2) # Lower quantile
        lines(start_year:(start_year+horizon - 1), LGD_quantiles_stationary_list_ratios_adj_cycle[3, 1:horizon], col = col_stat, lwd = 3, lty = 2) # Upper quantile
polygon(c(start_year:(start_year+horizon - 1), rev(start_year:(start_year+horizon - 1))), 
                c(LGD_quantiles_stationary_list_ratios_adj_cycle[1, 1:horizon], rev(LGD_quantiles_stationary_list_ratios_adj_cycle[3, 1:horizon])), 
                col = alpha(1, 0.1), border = NA) 
## Plot the legend: ----------
legend("topleft", legend = c("Stationary","SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = c(col_stat, col_list), cex = 0.9, lty = c(3, 1,1,1,1), lwd = rep(4, 5))
...
```{r Import the APRA statistics for calcualting solvency ratios}
APRA_market_stats_1 <- read_excel(GI_fins_stats_input, sheet = "Data - Level 1")
```

```{r Calculate solvency ratios for primary insurers based on APRA statistics}
## Extract the written premiums:
APRA_market_gross_premiums <- APRA_market_stats_1 %>% filter(`Data item` == "Gross written premium") %>% filter(`Industry sector` == "Direct insurer") %>% select(`Reporting date`, `Company short name`, Value) %>% drop_na() %>% rename(Gross_written_premium = Value) %>% filter(Gross_written_premium > 0) ## Exclude the insolvency case
## Extract the capital base
APRA_market_capital <- APRA_market_stats_1 %>% filter(`Data item` == "Capital base") %>% filter(`Industry sector` == "Direct insurer") %>% select(`Reporting date`, `Company short name`,  Value) %>% drop_na() %>% rename(Capital_base = Value) %>% filter(Capital_base > 0) ## Exclude the insolvency case
## Extract the Minimum Capital requirements: 
APRA_market_MCR <- APRA_market_stats_1 %>% filter(`Data item` == "Minimum capital requirement") %>% filter(`Industry sector` == "Direct insurer") %>% select(`Reporting date`, `Company short name`,  Value) %>% drop_na() %>% rename(MCR = Value) %>% filter(MCR > 0) ## Exclude the insolvency case
## Join the data: 
APRA_market_solvency <- left_join(APRA_market_gross_premiums, APRA_market_capital, by = c("Reporting date", "Company short name")) %>% left_join(APRA_market_MCR, by = c("Reporting date", "Company short name")) %>% mutate(Solvecny_ratio_capitalBase = Capital_base/Gross_written_premium, Solvency_ratio_MCR = MCR/Gross_written_premium, Solvency_multiplier = Capital_base/MCR)
```


```{r Derive the long-term steady-state values}
### Derive the solvency statistics: 
APRA_market_solvency_stats <- APRA_market_solvency %>% group_by(`Reporting date`) %>% summarise(Solvecny_ratio_capitalBase_mean = mean(Solvecny_ratio_capitalBase, na.rm = T), Solvency_ratio_MCR_mean = mean(Solvency_ratio_MCR, na.rm = T),Solvecny_ratio_capitalBase_median = median(Solvecny_ratio_capitalBase, na.rm = T), Solvency_ratio_MCR_median = median(Solvency_ratio_MCR, na.rm = T)) %>% mutate(Year = year(`Reporting date`), Month = month(`Reporting date`)) %>% filter(Month == 6) 
### Remove outliers:
APRA_market_solvency_stats_outlier_removed <- APRA_market_solvency_stats %>% filter(Solvecny_ratio_capitalBase_mean < 10)
### Calculate the long-term steady-state ratios: 
solvency_ratios_mean <- mean(APRA_market_solvency_stats_outlier_removed$Solvecny_ratio_capitalBase_mean, na.rm = T)
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Ratios_of_capital_base_to_written_Premiums.pdf")
plot(x = APRA_market_solvency_stats_outlier_removed$Year, y = APRA_market_solvency_stats_outlier_removed$Solvecny_ratio_capitalBase_mean, type = "p", pch = 16, xlab = "Years", ylab = "Ratios", main = "Ratios of capital base to gross written premiums (APRA statistics)", cex.main = 0.9)
abline(h = solvency_ratios_mean,  lwd = 2, lty = 2, col = "red")
legend("topleft", lty = 2, col = "red", legend = c("Mean"), lwd = 2)
#dev.off()
```

```{r}
paste("Selected long-run mean of capital-to-premium ratios:", round(solvency_ratios_mean, 0))
```



```{r Write a sensitivity testing function for dividend payout ratios}
Div_sensitivity_test_func <- function(payout_ratio, solvency_steady_state, Div_limit_param){             #######################################################
    # Calculate firm-level surplus:
    #######################################################
    ## Define empty list to store the outputs:
    capitalPremiums_sims_list_L_cycle <- list()
    capitalPremiums_sims_list_M_cycle <- list()
    ...
    for(s in 1:4){
        ...
        UL_sims_Large_cycle <- calc_surplus_func_full_cycle(Risk_free_weight = Risk_free_weight_default, 
                                      ...
                                      k3_param = 0,
                                      assumption_alternative = FALSE,
                                      Div_payout = payout_ratio, solvency_ratio_steady = solvency_steady_state, Div_limit = Div_limit_param)
    ...
    #######################################################
    # Calculate market-level surplus: 
    #######################################################
    ## Create empty lists: 
    capital_sims_list_Market_cycle <- list()
    for(s in 1:4){
...
for(s in 1:4){
    ## Derive quantiles for market capital: 
    market_capital_adj_quantiles_list_cycle[[s]] <-  apply(capital_sims_list_Market_adj_all_scenarios_cycle[[s]], MARGIN = 1, FUN = function(x) multi_func_withMed2(x, quantile_low = 0.005, quantile_high = 0.995))
    ## Calculate insolvency probabilities
    market_capital_adj_default_cycle[, s] <- apply(capital_sims_list_Market_adj_all_scenarios_cycle[[s]], MARGIN = 1, FUN = function(x) mean(x <= 0))
}
return(list(Market_capital_stats = market_capital_adj_quantiles_list_cycle, 
            Market_insolvency_rates = market_capital_adj_default_cycle))
}
```

```{r Perform the dividend payout ratios sensitivity test, warning=FALSE}
Div_payout_ratios_list <- c(0.1, 0.3, 0.5, 0.7, 0.9)
capital_stats_Div_payout_sensitivity_list <- list()
for(i in 1:length(Div_payout_ratios_list)){
    capital_stats_Div_payout_sensitivity_list[[i]] <- Div_sensitivity_test_func(Div_payout_ratios_list[i], solvency_steady_state = 3, Div_limit = 0.5)
}
```

```{r Perform the steady-state capital level sensitivity test under higher solvency steady state, warning=FALSE}
capital_stats_Div_payout_sensitivity_Divlimit1_list <- list()
for(i in 1:length(Div_payout_ratios_list)){
   capital_stats_Div_payout_sensitivity_Divlimit1_list[[i]] <- Div_sensitivity_test_func(Div_payout_ratios_list[i], solvency_steady_state = 3, Div_limit = 1)
}
```



```{r Define a list of line types used in the plotting}
lty_list <- c(5, 2, 6, 4)
```



```{r Plot the comparison of mean surplus under different dividend payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Mean_surplus_byPayoutRatios_Scenarios.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(s in 1:4){
plot(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_quantiles_list_cycle[[s]][2, 1:horizon]), type = "l", lwd = 2, col = col_list[s], xlab = "Years", ylab = "Mean surplus", main = paste("Mean market surplus (log):", scenario_list[s]), cex.main = 0.9)
for(j in 1:(length(Div_payout_ratios_list) - 1)){
    points(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[s]][4, 1:horizon]), type = "l", lwd = 2, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = paste("Payout ratios:", c(0,Div_payout_ratios_list[1:4])), lty = c(1, lty_list), lwd = 2, col = col_list[s], cex = 0.7)
}
#dev.off()
```

```{r Plot the comparison of ranking of mean surplus across different payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Mean_surplus_byScenarios_payoutRatios.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
plot(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[1]][4, 1:horizon]), type = "l", lwd = 3, col = col_list[1], xlab = "Years", ylab = "Mean surplus", main = paste("Mean market surplus (log) with payout ratio:", Div_payout_ratios_list[j]), cex.main = 0.9, lty = lty_list[j], ylim = c(25, 30))
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[s]][4, 1:horizon]), type = "l", lwd = 3, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, lwd = rep(2, 2), cex = 0.7, lty = lty_list[j])
}
#dev.off()
```


```{r Derive the average difference in mean surplus}
mean_diff_surplus_DivPayouts_list <- c()
mean_diff_surplus_DivPayouts_list[1] <- mean((market_capital_adj_quantiles_list_cycle[[1]][2, 1:horizon] - market_capital_adj_quantiles_list_cycle[[4]][2, 1:horizon])/market_capital_adj_quantiles_list_cycle[[1]][2, 1:horizon])
for(j in 1:4){
    mean_diff_surplus_DivPayouts_list[j + 1] <- mean((capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[1]][4, 1:horizon] - capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[4]][4, 1:horizon])/capital_stats_Div_payout_sensitivity_list[[j]]$Market_capital_stats[[1]][4, 1:horizon])
}

data.frame(Dividend_payout = c(0, Div_payout_ratios_list[1:4]), Mean_diff_surplus = mean_diff_surplus_DivPayouts_list)
```


```{r Plot the comparison of insolvency probabilities under different dividend payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/InsolvencyProbs_byPayoutRatios_Scenarios.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(s in 1:4){
plot(x = start_year:(start_year + horizon - 1), y = market_capital_adj_default_cycle[1:horizon, s], type = "l", lwd = 2, col = col_list[s], xlab = "Years", ylab = "Insolvency", main = paste("Insolvency probability:", scenario_list[s]), cex.main = 0.9, ylim = c(0, 0.4))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
    points(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon,s], type = "l", lwd = 2, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = paste("Payout ratios:", c(0,Div_payout_ratios_list[1:4])), lty = c(1, lty_list), lwd = 2, col = col_list[s], cex = 0.7)
}
#dev.off()
...
```{r Plot the comparison of ranking of insolvency probabilities across different payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/InsolvencyProbs_byScenarios_payoutRatios.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
plot(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon,1], type = "l", lwd = 3, col = col_list[1], xlab = "Years", ylab = "Mean surplus", main = paste("Insolvency probability with payout ratio:", Div_payout_ratios_list[j]), cex.main = 0.9, lty = lty_list[j], ylim = c(0, max(capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon, ])))
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon,s], type = "l", lwd = 3, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, lwd = rep(2, 2), cex = 0.7, lty = lty_list[j])
}
#dev.off()
...
```{r Derive the average difference in insolvency probability}
mean_diff_insolvency_probs_DivPayouts_list <- c()
mean_diff_insolvency_probs_DivPayouts_list[1] <- mean(market_capital_adj_default_cycle[1:horizon, 1] - market_capital_adj_default_cycle[1:horizon, 4])
for(j in 1:4){
    mean_diff_insolvency_probs_DivPayouts_list[j + 1] <- mean(capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon,1] - capital_stats_Div_payout_sensitivity_list[[j]]$Market_insolvency_rates[1:horizon,4])
}
## Plot the results:
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Mean_diff_insolvency_prob_SSP26vs85.pdf")
barplot(mean_diff_insolvency_probs_DivPayouts_list, ylim = c(0, 0.035), names.arg = c(0, Div_payout_ratios_list[1:4]), xlab = "Dividend payout ratios", ylab = "Difference", main = "Mean difference between insolvency probability (SSP 2.6 v.s SSP 8.5)", cex.main = 1)
#dev.off()

data.frame(Dividend_payout = c(0, Div_payout_ratios_list[1:4]), Mean_diff_insolvency = mean_diff_insolvency_probs_DivPayouts_list)
```

Derive the results with higher cap on dividends payout ($100\%$): 

```{r Plot the comparison of ranking of mean surplus across different payout ratios with lower cap}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Mean_surplus_byPayoutRatios_Scenarios_HigherCap.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(s in 1:4){
plot(x = start_year:(start_year + horizon - 1), y = log(market_capital_adj_quantiles_list_cycle[[s]][2, 1:horizon]), type = "l", lwd = 2, col = col_list[s], xlab = "Years", ylab = "Mean surplus", main = paste("Mean market surplus (log):", scenario_list[s]), cex.main = 0.9)
for(j in 1:(length(Div_payout_ratios_list) - 1)){
    points(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_capital_stats[[s]][4, 1:horizon]), type = "l", lwd = 2, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = paste("Payout ratios (with 100% cap):", c(0,Div_payout_ratios_list[1:4])), lty = c(1, lty_list), lwd = 2, col = col_list[s], cex = 0.7)
}
#dev.off()
```
...
```{r Plot the comparison of ranking of mean surplus across different payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/Mean_surplus_byScenarios_payoutRatios_HigherCap.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
plot(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_capital_stats[[1]][4, 1:horizon]), type = "l", lwd = 3, col = col_list[1], xlab = "Years", ylab = "Mean surplus", main = paste("Mean market surplus (log) with payout ratio (100% cap):", Div_payout_ratios_list[j]), cex.main = 0.85, lty = lty_list[j], ylim = c(25, 30))
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = log(capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_capital_stats[[s]][4, 1:horizon]), type = "l", lwd = 3, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, lwd = rep(2, 2), cex = 0.7, lty = lty_list[j])
}
#dev.off()
...
```{r Plot the comparison of insolvency probabilities under different dividend payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/InsolvencyProbs_byPayoutRatios_Scenarios_HigherCap.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(s in 1:4){
plot(x = start_year:(start_year + horizon - 1), y = market_capital_adj_default_cycle[1:horizon, s], type = "l", lwd = 2, col = col_list[s], xlab = "Years", ylab = "Insolvency", main = paste("Insolvency probability:", scenario_list[s]), cex.main = 0.9, ylim = c(0, 0.4))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
    points(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_insolvency_rates[1:horizon,s], type = "l", lwd = 2, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = paste("Payout ratios (with 100% cap):", c(0,Div_payout_ratios_list[1:4])), lty = c(1, lty_list), lwd = 2, col = col_list[s], cex = 0.7)
}
#dev.off()
...
```{r Plot the comparison of ranking of insolvency probabilities across different payout ratios}
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Surplus/InsolvencyProbs_byScenarios_payoutRatios_HigherCap.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(j in 1:(length(Div_payout_ratios_list) - 1)){
plot(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_insolvency_rates[1:horizon,1], type = "l", lwd = 3, col = col_list[1], xlab = "Years", ylab = "Mean surplus", main = paste("Insolvency probability with payout ratio (100% cap):", Div_payout_ratios_list[j]), cex.main = 0.85, lty = lty_list[j], ylim = c(0, max(capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_insolvency_rates[1:horizon, ])))
for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y = capital_stats_Div_payout_sensitivity_Divlimit1_list[[j]]$Market_insolvency_rates[1:horizon,s], type = "l", lwd = 3, col = col_list[s], lty = lty_list[j])
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, lwd = rep(2, 2), cex = 0.7, lty = lty_list[j])
}
#dev.off()
...
## Economic damage sensitivity test

### Calibrate the adjustment parameters

```{r Define the DICE damage function}
calc_damage_ratios_DICE <- function(GMST, gamma = 0.003467, exponent = 2){
    damage_ratio <- gamma * GMST^exponent
    return(damage_ratio)
}

calc_damage_ratios_DICE2016 <- function(GMST, gamma = 0.00227, exponent = 2){
    damage_ratio <- gamma * GMST^exponent
    return(damage_ratio)
} # Source from: https://www.nber.org/system/files/working_papers/w22933/w22933.pdf

calc_damage_ratios_DICE1992 <- function(GMST, gamma = 0.0133, exponent = 2, divisor = 3){
    damage_ratio <- gamma * (GMST/divisor)^exponent
    return(damage_ratio)
} # Source from: https://stephenschneider.stanford.edu/Publications/PDF_Papers/OptimalScience1192.pdf
```

```{r Import the GMST increase data}
SSP_temp_dat <- read.csv("Supplementary data/Temperature increase/global-average-temperature-increase-relative-to-the-pre-industrial-era.csv") %>% pivot_wider(names_from = Scenario, values_from = Temperature)
colnames(SSP_temp_dat) = c("Year","SSP 1.00", "SSP 2.00", "SSP 3.00", "SSP 5.00")
#### Interpolation: 
interpolated_years_temp <- min(SSP_temp_dat$Year):max(SSP_temp_dat$Year)
SSP_temp_dat_interpolate_dat <- data.frame(Year = interpolated_years_temp,
                                              `SSP 1.00` = spline(SSP_temp_dat$Year, SSP_temp_dat$`SSP 1.00`, n = length(interpolated_years_temp))$y,
                                              `SSP 2.00` = spline(SSP_temp_dat$Year, SSP_temp_dat$`SSP 2.00`, n = length(interpolated_years_temp))$y,
                                              `SSP 3.00` = spline(SSP_temp_dat$Year, SSP_temp_dat$`SSP 3.00`, n = length(interpolated_years_temp))$y,
                                              `SSP 5.00` = spline(SSP_temp_dat$Year, SSP_temp_dat$`SSP 5.00`, n = length(interpolated_years_temp))$y) %>% filter(Year >= 2023)
colnames(SSP_temp_dat_interpolate_dat) = c("Year", scenario_list)
```

```{r Derive the DICE damage ratios}
damage_ratios_DICE_mat <- matrix(NA, nrow = nrow(SSP_temp_dat_interpolate_dat), ncol = 4)
for(s in 1:4){
    damage_ratios_DICE_mat[, s] <- calc_damage_ratios_DICE(GMST = SSP_temp_dat_interpolate_dat[, s + 1])
}
damage_ratios_DICE2016_mat <- matrix(NA, nrow = nrow(SSP_temp_dat_interpolate_dat), ncol = 4)
for(s in 1:4){
    damage_ratios_DICE2016_mat[, s] <- calc_damage_ratios_DICE2016(GMST = SSP_temp_dat_interpolate_dat[, s + 1])
}
damage_ratios_DICE1992_mat <- matrix(NA, nrow = nrow(SSP_temp_dat_interpolate_dat), ncol = 4)
for(s in 1:4){
    damage_ratios_DICE1992_mat[, s] <- calc_damage_ratios_DICE1992(GMST = SSP_temp_dat_interpolate_dat[, s + 1])
}


### Plot the implied DICE damage ratios: 
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Asset/Implied_SSP_damage_ratios.pdf")
plot(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE_mat[1:horizon, 1], col = col_list[1], type = "l", ylim = c(0, 0.04), xlab = "Years", ylab = "Damage ratios", main = "Implied SSP damage ratios from DICE coefficients", lwd = 3, yaxt = "n")
points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE2016_mat[1:horizon, 1], col = col_list[1], type = "l", lwd = 3, lty = 5)
points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE1992_mat[1:horizon, 1], col = col_list[1], type = "l", lwd = 6, lty = 3)
axis(2, at = pretty(c(0, 0.04)), labels = paste0(pretty(c(0, 0.04)) * 100, "%"))
for(s in 2:4){
    points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE_mat[1:horizon, s], col = col_list[s], type = "l", lwd = 3)
    points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE2016_mat[1:horizon, s], col = col_list[s], type = "l", lwd = 3, lty = 5)
    points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = damage_ratios_DICE1992_mat[1:horizon, s], col = col_list[s], type = "l", lwd = 6, lty = 3)
}
legend("topleft", legend =c(paste(scenario_list, "(DICE 2023)"), paste(scenario_list, "(DICE 2016R2)"), paste(scenario_list, "(DICE 1992)")), col = col_list, cex = 0.7, lty = c(rep(1, 4), rep(2, 4), rep(3, 4)), lwd = 1.5)

#dev.off()

## Plot the GDP pathway adjusted for the climate damage:
#pdf("/Users/jimli/Documents/DFA codes/Plotting/Asset/DICE_adjusted_GDP.pdf")
plot(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = Real_GDP_forecasts_all[1:horizon, 4], type = "l", lty = 1, col = col_list[4], lwd = 3, xlab = "Years", ylab = "Real GDP", main = "Real GDP (Australia): Raw vs. Climate-damage-adjusted (DICE coefficients)", cex.main = 0.9, ylim = range(Real_GDP_forecasts_all[1:horizon, ]))

points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y =Real_GDP_forecasts_all[1:horizon, 4]*(1 - damage_ratios_DICE_mat[1:horizon, 4]), type = "l", lty = 4, col = col_list[4], lwd = 3)


#points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y =Real_GDP_forecasts_all[1:horizon, 4]*(1 - damage_ratios_DICE2016_mat[1:horizon, 4]), type = "l", lty = 6, col = col_list[4], lwd = 3)

#points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y =Real_GDP_forecasts_all[1:horizon, 4]*(1 - Total_damage_ratio_quantiles_list[[4]][2, 1:horizon]), type = "l", lty = 3, col = col_list[4], lwd = 2)
for(s in 1:3){
    points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y = Real_GDP_forecasts_all[1:horizon, s], type = "l", lty = 1, col = col_list[s], lwd = 3)
    points(x = SSP_temp_dat_interpolate_dat$Year[1:horizon], y =Real_GDP_forecasts_all[1:horizon, s]*(1 - damage_ratios_DICE_mat[1:horizon, s]), type = "l", lty = 4, col = col_list[s], lwd = 3)
}
legend("topleft", legend =c(paste(scenario_list, "(Raw)"), paste(scenario_list, "(Damage-adjusted; DICE 2023)")), col = col_list, cex = 0.6, lty = c(rep(1, 4), rep(2, 4)), lwd = 2)
#dev.off()
```
...
```{r Calibrate using the linear regression approach}
DICE_damage_scalar_alpha_list <- c()
DICE_damage_scalar_beta_list <- c()
for(s in 1:4){
    fit_lm <- lm(sort(damage_ratios_DICE_mat[1:horizon, s]) ~ sort(Insure_damage_ratio_quantiles_list[[s]][2, 1:horizon]))
    DICE_damage_scalar_alpha_list[s] <- fit_lm$coefficients[1]
    DICE_damage_scalar_beta_list[s] <- fit_lm$coefficients[2]
}
```


```{r Derive the adjusted total damage ratios}
Total_damage_ratio_DICEAdj_quantiles_list <- list()
for(s in 1:4){
    Total_damage_ratio_DICEAdj_quantiles_list[[s]] <- DICE_damage_scalar_alpha_list[s] + Insure_damage_ratio_quantiles_list[[s]] *  DICE_damage_scalar_beta_list[s]
}
```



```{r Plot the bias correction results for the total damage ratios}
## Plot the original total economic damage ratios: 
plot_quantiles_scenario_2(Insure_damage_ratio_quantiles_list, forecast_periods = start_year:(start_year+horizon - 1), var_name = "Damage ratios", main = "Ratios of insurance loss to GDP", y_range = c(0, 0.015), colour_list = col_list, horizon = horizon, transparency = 0.1, label_percents = T, step_plot_perc = 0.005, quantile_lines = F, lwd_mean = 4)
## Bias correction: 
#pdf("Plotting/Asset/Mean_damage_ratios_2060_DICE_QuantileMapping.pdf")
par(mar = c(2.4, 2.4, 2.4, 1), mgp = c(1.5, 0.5, 0))
par(mfrow = c(2, 2))
for(s in 1:4){
range_ratios_plot <- c(0, max(Total_damage_ratio_DICEAdj_quantiles_list[[s]][2, 1:horizon]))
plot(x = sort(damage_ratios_DICE_mat[1:horizon, s]), y = sort(Total_damage_ratio_DICEAdj_quantiles_list[[s]][2, 1:horizon]), col = col_list[s], ylim = range_ratios_plot, yaxt = "n", xlab = "Sorted DICE-implied damage ratios", ylab = "Sorted adjusted damage ratios", main = scenario_list[s], cex.main = 0.9, pch = 16, cex.lab = 0.9)
#points(x = sort(damage_ratios_DICE_mat[1:horizon, s]), y = sort(Insure_damage_ratio_quantiles_list[[s]][2, 1:horizon]), col = col_list[s], pch = 16)
abline(coef = c(0,1), lwd = 2, lty = 2)
axis(2, at = pretty(range_ratios_plot), labels = paste0(pretty(range_ratios_plot) * 100, "%"))
}
#dev.off()
```

### Perform the simulations based on the adjusted economic growth

```{r Simulated the equity return based on the corporate earnings channels based on the adjusted economic growth}
equity_TR_excess_adj_OP_DICE_Adj_list <- list()
equity_TR_excess_adj_OP_brown_DICE_Adj_list <- list()
equity_TR_excess_adj_OP_base_DICE_Adj_list <- list()
equity_TR_cum_excess_adj_OP_DICE_Adj_list <- list()
equity_TR_cum_excess_adj_OP_brown_DICE_Adj_list <- list()
DICE_adj_economic_damage_list <- list()
#DICE_adj_economic_growth_list <- list()
...
TR_sims_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
TR_sims_brown_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
DICE_adj_economic_damage_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
DICE_adj_economic_growth_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
Random_shocks_OP_mat <- Random_shocks_OP_list[[s]]
Random_shocks_TR_op_mat <- Random_shocks_TR_op_list[[s]]
for(i in 1:n.sims){
    ## Get the damage ratios:
    damage_ratio <- pmin(DFA_components_all_scenarios[[s]]$Liabilities_module$CAT_damage_ratio[, i], 0.999)
    economic_damage_ratio <- DICE_damage_scalar_alpha_list[s] + damage_ratio *  DICE_damage_scalar_beta_list[s]
    #economic_damage_ratio_adj <- damage_ratio * Total_damage_scalar_mean
    #economic_damage_ratio_adj <- economic_damage_ratio
    economic_damage_ratio_adj <- pmax(economic_damage_ratio, 0)
    ## Constraint the damage ratio to be less than 100% of GDP
    ## Derive the adjusted GDP:
    Real_GDP_pred_adj <- c(Real_GDP_pred_raw[1:2], Real_GDP_pred_raw[3:(3+forecast_horizon-1)]*(1-economic_damage_ratio_adj))
    Real_GDP_pred_adj_growth <- na.omit(Real_GDP_pred_adj/lag(Real_GDP_pred_adj)-1)[-1]
    ## Derive the total equity returns:
    ### Simulate the random shocks:
    epsilon_op <- Random_shocks_OP_mat[1:forecast_horizon, i]
    epsilon_TR <- Random_shocks_TR_op_mat[1:forecast_horizon, i]
    
    ### Simulate the operating profit growth:
    #### Derive the change in operating profit for the general sector:
    #OP <-  Real_GDP_pred_adj_growth + epsilon_op
    OP <- lm_OP_consumption_growth$coefficients[1] + lm_OP_consumption_growth$coefficients[2]*Real_GDP_pred_adj_growth + epsilon_op
    #### Derive the change in operating profit for the brown sector:
    OP_brown <- OP + Brown_production_change*scalar_impact_WDS_EBIT
    ### Simulate the total excess returns:
    #### General sector:
    #TR <- pmax(OP + epsilon_TR,-1)
    TR <- pmax(intercept_OP_TR + beta_OP_TR * OP + epsilon_TR, -1)
    TR_cum <- cumprod(1 + TR) - 1
    #### Brown sector:
    #TR_brown <- pmax(OP_brown + epsilon_TR,-1)
    TR_brown <- pmax(intercept_OP_TR + beta_OP_TR * OP_brown + epsilon_TR,-1)
    TR_brown_cum <- cumprod(1 + TR_brown) - 1
    ...
    TR_sims_cum_mat[, i] <- TR_cum
    TR_sims_brown_cum_mat[, i] <- TR_brown_cum 
    DICE_adj_economic_damage_mat[, i] <- economic_damage_ratio_adj
    DICE_adj_economic_growth_mat[, i] <- Real_GDP_pred_adj_growth
}
equity_TR_excess_adj_OP_DICE_Adj_list[[s]] <- TR_sims_mat
equity_TR_excess_adj_OP_brown_DICE_Adj_list[[s]] <- TR_sims_mat_Brown
equity_TR_excess_adj_OP_base_DICE_Adj_list[[s]] <- TR_sims_mat_base
equity_TR_cum_excess_adj_OP_DICE_Adj_list[[s]] <- TR_sims_cum_mat
equity_TR_cum_excess_adj_OP_brown_DICE_Adj_list[[s]] <- TR_sims_brown_cum_mat
DICE_adj_economic_damage_list[[s]] <- DICE_adj_economic_damage_mat
#DICE_adj_economic_growth_list[[s]] <- DICE_adj_economic_growth_mat
}
```


```{r Also derive the adjusted consumption growth based on the DICE-adjusted damage ratios}
DICE_adj_economic_growth_list <- list() 
for(s in 1:4){
    Real_GDP_pred_raw <- Real_GDP_forecasts_all[, s]
    DICE_adj_economic_growth_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    for(i in 1:n.sims){
    ## Get the damage ratios:
    damage_ratio <- pmin(DICE_adj_economic_damage_list[[s]][, i], 0.999)
    ## Constraint the damage ratio to be less than 100% of GDP
    ## Derive the adjusted GDP:
    Real_GDP_pred_adj <- c(Real_GDP_pred_raw[1:2], Real_GDP_pred_raw[3:(3+forecast_horizon-1)]*(1-damage_ratio))
    Real_GDP_pred_adj_growth <- na.omit(Real_GDP_pred_adj/lag(Real_GDP_pred_adj)-1)[-1]
    DICE_adj_economic_growth_mat[, i] <- Real_GDP_pred_adj_growth
    }
DICE_adj_economic_growth_list[[s]] <- DICE_adj_economic_growth_mat
...
```{r Calculation of cumulative investment returns based on adjusted damage ratios}
investment_returns_real_all_scenarios_DICE_Adj_list <- list()
investment_returns_nominal_all_scenarios_DICE_Adj_list <- list()
investment_returns_rates_nominal_all_scenarios_DICE_Adj_list <- list()
log_investment_returns_nominal_all_scenarios_DICE_Adj_list <- list()
log_equity_returns_nominal_all_scenarios_DICE_Adj_list <- list()
log_consumption_cum_growth_all_scenarios_DICE_Adj_list <- list()
for(s in 1:4){
    investment_returns_real_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    investment_returns_nominal_cum_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    log_investment_returns_nominal_cum_mat  <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    log_equity_returns_nominal_cum_mat  <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    investment_returns_nominal_mat <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    log_consumption_cum_growth_mat  <- matrix(NA, nrow = forecast_horizon, ncol = n.sims)
    
    for(i in 1:n.sims){
         ## Get the inflation rates:
         Short_rates_real <-  DFA_components_all_scenarios[[s]]$MEV_module$Real_rates[, i]
         Short_rates_nominal <- DFA_components_all_scenarios[[s]]$MEV_module$Nominal_rates[, i]
         adj_eco_growth <-  pmax(DICE_adj_economic_growth_list[[s]][, i], -0.99999)
         
         ## Get the brown and other excess return:
         brown_return <- equity_TR_excess_adj_OP_brown_DICE_Adj_list[[s]][, i]
         other_return <- equity_TR_excess_adj_OP_DICE_Adj_list[[s]][, i]
         ...
         log_investment_returns_nominal_cum_mat[, i] <- log(cumprod(1 + investment_return_nominal))
         log_equity_returns_nominal_cum_mat[, i] <- log(cumprod(1 + equity_return_nominal))
         log_consumption_cum_growth_mat[, i] <- log(cumprod(1 + adj_eco_growth))
    }
    
investment_returns_real_all_scenarios_DICE_Adj_list[[s]] <- investment_returns_real_cum_mat
investment_returns_nominal_all_scenarios_DICE_Adj_list[[s]] <- investment_returns_nominal_cum_mat
investment_returns_rates_nominal_all_scenarios_DICE_Adj_list[[s]] <- investment_returns_nominal_mat
log_investment_returns_nominal_all_scenarios_DICE_Adj_list[[s]] <- log_investment_returns_nominal_cum_mat
log_equity_returns_nominal_all_scenarios_DICE_Adj_list[[s]] <- log_equity_returns_nominal_cum_mat
log_consumption_cum_growth_all_scenarios_DICE_Adj_list[[s]] <- log_consumption_cum_growth_mat
}
```
...
```{r Calcualte the quantiles statistics of cummulative investment returns}
investment_returns_real_quantiles_all_scenarios_DICE_Adj_list <- list()
investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list <- list()
log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list <- list()
log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list <- list()
DICE_adj_economic_damage_quantiles_all_scenarios_list <- list()
log_consumption_cum_growth_quantiles_all_scenarios_DICE_Adj_list <- list()
for(s in 1:4){
    ## Calculate real cumulative investment returns:
    investment_returns_real_quantiles_all_scenarios_DICE_Adj_list[[s]] <-  apply(investment_returns_real_all_scenarios_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
    ## Calculate nominal cumulative investment returns:
    investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]] <-  apply(investment_returns_nominal_all_scenarios_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
    ## Calculate log compounded investment returns:
    log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]] <- apply(log_investment_returns_nominal_all_scenarios_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
     ## Calculate log compounded equity returns:
    log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]] <- apply(log_equity_returns_nominal_all_scenarios_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x[is.finite(x) == T], quantile_low = 0.05, quantile_high = 0.95))
    ## Calculate statistics for adjusted economic growth: 
    DICE_adj_economic_damage_quantiles_all_scenarios_list[[s]] <- apply(DICE_adj_economic_damage_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
    ## Calculate statistics for cumulative adjusted economic growth:
    log_consumption_cum_growth_quantiles_all_scenarios_DICE_Adj_list[[s]] <- apply(log_consumption_cum_growth_all_scenarios_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
}
```

```{r Derive the summary statistic for the excess returns}
equity_TR_excess_adj_OP_DICE_Adj_quantiles_all_scenarios_list <- list()
equity_TR_excess_adj_OP_Adj_quantiles_all_scenarios_list <- list()
for(s in 1:4){
    equity_TR_excess_adj_OP_DICE_Adj_quantiles_all_scenarios_list[[s]] <- apply(equity_TR_excess_adj_OP_DICE_Adj_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
   equity_TR_excess_adj_OP_Adj_quantiles_all_scenarios_list[[s]] <-  apply(equity_TR_excess_adj_OP_list[[s]], MARGIN = 1, FUN = function(x) multi_func(x, quantile_low = 0.05, quantile_high = 0.95))
}
```
...
```{r Compare the adjusted economic growth with DICE}
#pdf("Plotting/Asset/Mean_damage_ratios_2060_DICE_AdjvsUnAdj.pdf")
## Plot the mean adjusted total damage ratios:
plot(x = start_year:(start_year+horizon - 1),  y = DICE_adj_economic_damage_quantiles_all_scenarios_list[[1]][2, 1:horizon], col = col_list[1], lwd = 3, type = "l", ylim = c(0, 0.04), xlab = "Forecast periods", ylab = "Damage ratios", main = "Adjusted v.s DICE-implied damage ratios", yaxt = "n")
axis(2, at = pretty(c(0, 0.04)), labels = paste0(pretty(c(0, 0.04)) * 100, "%"))
for(s in 2:4){
    points(x = start_year:(start_year+horizon - 1),  y = DICE_adj_economic_damage_quantiles_all_scenarios_list[[s]][2, 1:horizon], col = col_list[s], lwd = 3, type = "l")
}
for(s in 1:4){
    points(x = start_year:(start_year+horizon - 1), y = damage_ratios_DICE_mat[1:horizon, s], lwd = 3, lty = 2, type = "l", col = col_list[s])
}
legend("topleft", legend = c(paste(scenario_list, "(Adjusted)"), paste(scenario_list, "(DICE)")), col = rep(col_list, times = 2), cex = 0.6, lty = c(rep(1, 4), rep(2, 4)), lwd = 2)
#dev.off()
```

```{r Plot the quantile range of log-sale of cumulative investment returns based on the adjusted economic growth, fig.height = 3}
horizon <- 38
#pdf("Plotting/Asset/Simulations_cum_investment_returns_2060_DICE_adj.pdf")
plot_quantiles_scenario(log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Cumulative returns (log)", main = "Cumulative investment returns (log-scale; adjusted)", y_range = c(-0.2, 5), colour_list = col_list, horizon = horizon , transparency = 0.1, label_percents = F, cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, mean_lwd = 4, quantiles_lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()

## Comparison of mean cumulative investment returns: 
#pdf("Plotting/Asset/Mean_simulations_cum_investment_returns_2060_DICE_AdjvsUnAdj.pdf")
plot(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_list[[1]][2, 1:horizon], col = alpha(col_list[1], 0.7), type = "l", lwd = 4, ylim = c(0, 5), xlab = "Years", ylab = "Cumulative returns (log)", main = "Mean log cumulative returns (Unadjusted vs. adjusted for DICE damage ratios)", cex.lab = 1.5, )
points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[1]][2, 1:horizon], col = col_list[1], type = "l", lwd = 3, lty = 4, cex.axis = 1.5)
for(s in 2:4){
    points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][2, 1:horizon], col = alpha(col_list[s], 0.7), type = "l", lwd = 4)
    points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][2, 1:horizon], col = col_list[s], type = "l", lwd = 3, lty = 4)
}
legend("topleft", legend = c(paste(scenario_list, "(Unadjusted)"), paste(scenario_list, "(Adjusted)")), col = rep(col_list, times = 2), cex = 0.9, lty = c(rep(1, 4), rep(4, 4)), lwd = 2)
#dev.off()

## Plot the relative deviations:
#pdf("Plotting/Asset/Simulations_cum_investment_returns_diff_DICE_adjvsUnadj.pdf")
plot(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[1]][2, 2:(horizon+1)]/log_investment_returns_nominal_quantiles_all_scenarios_list[[1]][2, 2:(horizon+1)] - 1, col = col_list[1], type = "l", lwd = 4, ylim = c(-0.01, 0.01), yaxt = "n", xlab = "Years", ylab = "Relative difference (%)", main = "Relative difference in (log) cumulative investment returns (DICE-adjusted v.s. DICE-unadjusted)", cex.main = 0.85)
axis(2, at = pretty(c(-0.01, 0.01)), labels = paste0(pretty(c(-0.01, 0.01)) * 100, "%"))
for(s in 2:4){
    points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][2, 2:(horizon+1)]/log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][2, 2:(horizon+1)] - 1, col = col_list[s], type = "l", lwd = 4)
}
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()

## Comparison of lower bound of cumulative investment returns: 
#pdf("Plotting/Asset/LowerBound_simulations_cum_investment_returns_2060_DICE_AdjvsUnAdj.pdf")
# plot(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_list[[1]][1, 1:horizon], col = col_list[1], type = "l", lwd = 4, ylim = c(0, 5), xlab = "Years", ylab = "Cumulative returns (log)", main = "Mean log cumulative returns (Unadjusted vs. adjusted)", cex.lab = 1.5)
# points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[1]][1, 1:horizon], col = col_list[1], type = "l", lwd = 4, lty = 4, cex.axis = 1.5)
# for(s in 2:4){
#     points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][1, 1:horizon], col = col_list[s], type = "l", lwd = 4)
#     points(x = start_year:(start_year+ horizon - 1), y = log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][1, 1:horizon], col = col_list[s], type = "l", lwd = 4, lty = 4)
# }
# legend("topleft", legend = c(paste(scenario_list, "(Unadjusted)"), paste(scenario_list, "(Adjusted)")), col = rep(col_list, times = 2), cex = 0.9, lty = c(rep(1, 4), rep(4, 4)), lwd = 2)
#dev.off()
```

```{r Derive the average impact}
investment_returns_DICE_adj_mean_impact <- c()
for(s in 1:4){
    investment_returns_DICE_adj_mean_impact[s] <- mean(log_investment_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][2, ]/log_investment_returns_nominal_quantiles_all_scenarios_list[[s]][2, ] - 1)
}
```


```{r Plot the quantile range of log-sale of cumulative equity returns based on the adjusted economic growth}
#pdf("Plotting/Asset/Simulations_cum_equity_returns_2060_DICE_adj.pdf")
plot_quantiles_scenario(log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list, forecast_periods = start_year:(start_year+ horizon - 1), var_name = "Cumulative returns (log)", main = "Cumulative investment returns (log-scale; adjusted for DICE damage ratios)", y_range = c(-0.2, 7), colour_list = col_list, horizon = horizon , transparency = 0.1, label_percents = F, cex.main = 0.95, cex.lab = 1, cex.axis = 1, mean_lwd = 4, quantiles_lwd = 2)
legend("topleft", legend = c("SSP 2.6", "SSP 4.5", "SSP 7.0", "SSP 8.5"), col = col_list, cex = 1, lty = c(1,1,1,1), lwd = rep(4, 4))
#dev.off()

## Comparison of mean cumulative simulated returns: 
# plot(log_equity_returns_nominal_cum_quantiles_all_scenarios_list[[1]][2, 1:horizon], col = col_list[1], type = "l", lwd = 4)
# points(log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[1]][2, 1:horizon], col = col_list[1], type = "l", lwd = 4, lty = 2)
# for(s in 2:4){
#     points(log_equity_returns_nominal_cum_quantiles_all_scenarios_list[[s]][2, 1:horizon], col = col_list[s], type = "l", lwd = 4)
#     points(log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][2, 1:horizon], col = col_list[s], type = "l", lwd = 4, lty = 2)
# }
# legend("topleft", legend = c(paste(scenario_list, "(Unadjusted)"), paste(scenario_list, "(Adjusted)")), col = rep(col_list, times = 2), cex = 0.9, lty = c(rep(1, 4), rep(4, 4)), lwd = 2)
# # 
# plot(log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[1]][2, 2:(horizon+1)]/log_equity_returns_nominal_cum_quantiles_all_scenarios_list[[1]][2, 2:(horizon+1)] - 1, col = col_list[1], type = "l", lwd = 4, ylim = c(-0.02, 0.02))
# for(s in 2:4){
#     points(log_equity_returns_nominal_quantiles_all_scenarios_DICE_Adj_list[[s]][2, 2:(horizon+1)]/log_equity_returns_nominal_cum_quantiles_all_scenarios_list[[s]][2, 2:(horizon+1)] - 1, col = col_list[s], type = "l", lwd = 4)
# }
```

```{r Comparison of excess equity returns}
# plot(equity_TR_excess_adj_OP_DICE_Adj_quantiles_all_scenarios_list[[1]][2, 1:horizon]/equity_TR_excess_adj_OP_Adj_quantiles_all_scenarios_list[[1]][2, 1:horizon] - 1, type = "l", col = col_list[1])
# for(s in 2:4){
# points(equity_TR_excess_adj_OP_DICE_Adj_quantiles_all_scenarios_list[[s]][2, 1:horizon]/equity_TR_excess_adj_OP_Adj_quantiles_all_scenarios_list[[s]][2, 1:horizon] - 1, type = "l", col = col_list[s])
# }
```


```{r Comparison of compounded economic growth}
## Plot of compounded economic growth:
#pdf("Plotting/Asset/Log_compound_economic_growth_benchmarking.pdf")
plot(x = start_year:(start_year + horizon - 1), y =  log_gdp_potential_cum_growth_mat[1:horizon, 1], col = col_list[1], type = "l", ylim = c(0, 1.5), xlab = "Years", ylab = "Compounded growth (log)", main = "Cumulative economic growth (log-scale; adjusted)", cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.5, lwd = 1.5)
## Plot simulated cumulative growth: 
points(x = start_year:(start_year + horizon - 1), y = log_consumption_cum_growth_quantiles_all_scenarios_list[[1]][2, 1:horizon], col = col_list[1], type = "l", lty = 2, lwd = 1.5)
## Plot DICE adjusted growth:
points(x = start_year:(start_year + horizon - 1), y = log_consumption_cum_growth_quantiles_all_scenarios_DICE_Adj_list[[1]][2, 1:horizon], col = col_list[1], type = "l", lty = 4, lwd = 1.5)

for(s in 2:4){
    points(x = start_year:(start_year + horizon - 1), y =  log_gdp_potential_cum_growth_mat[1:horizon, s], col = col_list[s], type = "l", lwd = 1.5)
    points(x = start_year:(start_year + horizon - 1), y = log_consumption_cum_growth_quantiles_all_scenarios_list[[s]][2, 1:horizon], col = col_list[s], type = "l", lty = 2, lwd = 1.5)
    points(x = start_year:(start_year + horizon - 1), y = log_consumption_cum_growth_quantiles_all_scenarios_DICE_Adj_list[[s]][2, 1:horizon], col = col_list[s], type = "l", lty = 4, lwd = 1.5)
}
legend("topleft", legend = c(paste(scenario_list, "(raw)"), paste(scenario_list, "(Damage-adjusted)"), paste(scenario_list, "(Damage-adjusted; scaled for DICE)")), col = col_list, cex = 0.6, lty = c(rep(1, 4), rep(2, 4), rep(4, 4)), lwd = rep(1.5, 4))
#dev.off()
```

### Benchmarking of damage ratios with alternative study


```{r Import the IAM data}
# The IAM data are sourced from: https://purl.stanford.edu/wb587wt4560
IAM_damage_data <- read.csv("Supplementary data/IAMdata/ProcessedKoppData.csv")
SSP_temp_dat2060 <- SSP_temp_dat %>% filter(Year == 2060)
IAM_damage_data_PAGE <- IAM_damage_data %>% filter(IAM == "PAGE") %>% filter(abs(`T` - SSP_temp_dat2060$`SSP 1.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 1.00`)) | abs(`T` - SSP_temp_dat2060$`SSP 5.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 5.00`)))
IAM_damage_data_FUND <- IAM_damage_data %>% filter(IAM == "FUND") %>% filter(abs(`T` - SSP_temp_dat2060$`SSP 1.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 1.00`)) | abs(`T` - SSP_temp_dat2060$`SSP 5.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 5.00`)))
IAM_damage_data_PAGE <- IAM_damage_data %>% filter(IAM == "PAGE") %>% filter(abs(`T` - SSP_temp_dat2060$`SSP 1.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 1.00`)) | abs(`T` - SSP_temp_dat2060$`SSP 5.00`) == min(abs(`T` - SSP_temp_dat2060$`SSP 5.00`)))
    
    #filter(round(`T`, 0) == round(SSP_temp_dat2060$`SSP 1.00`, 0) | round(`T`, 1) == round(SSP_temp_dat2060$`SSP 5.00`, 1))

```


```{r Compare the alternative damage ratios}
horizon_bm <- 38
# Summarise the damage ratios and damage-adjusted GDP: 
damage_ratios_SSP26_bm <- c(IAM_damage_data_FUND$damages[1]/100,IAM_damage_data_PAGE$damages[1]/100,damage_ratios_DICE2016_mat[horizon_bm, 1], damage_ratios_DICE_mat[horizon_bm, 1], NA)
damage_ratios_SSP85_bm <- c(IAM_damage_data_FUND$damages[2]/100,IAM_damage_data_PAGE$damages[2]/100, damage_ratios_DICE2016_mat[horizon_bm, 4], damage_ratios_DICE_mat[horizon_bm, 4], 0.2)
## Summarise damage ratios: 
damage_ratios_benchmark_data_2060 <- data.frame(Models = c("FUND 3.8","PAGE09", "DICE-2016R2", "DICE-2023", "BHM-2015"), Damage_ratio_SSP26 = damage_ratios_SSP26_bm, Damage_ratio_SSP85 = damage_ratios_SSP85_bm)
## Summarise adjusted GDP for climate damage: 
GDP_benchmark_data_2060 <- data.frame(Models = c("Raw","FUND 3.8","PAGE09", "DICE-2016R2", "DICE-2023",  "BHM-2015"), GDP_adjusted_SSP26 = Real_GDP_forecasts_all[horizon_bm, ]$`SSP 1.00`*c(1, 1 - damage_ratios_SSP26_bm), GDP_adjusted_SSP85 = Real_GDP_forecasts_all[horizon_bm, ]$`SSP 5.00`*c(1, 1 - damage_ratios_SSP85_bm))

# Convert it to damage ratios: 
damage_ratios_benchmark_mat_2060 <- as.matrix(damage_ratios_benchmark_data_2060[, -1])
rownames(damage_ratios_benchmark_mat_2060) = damage_ratios_benchmark_data_2060$Models
colnames(damage_ratios_benchmark_mat_2060) = c("SSP 2.6", "SSP 8.5")

GDP_benchmark_mat_2060 <- as.matrix(GDP_benchmark_data_2060[, -1])
rownames(GDP_benchmark_mat_2060) = GDP_benchmark_data_2060$Models
colnames(GDP_benchmark_mat_2060) = c("SSP 2.6", "SSP 8.5")

# Generate barplot: 
## Plot the projected damage ratios: 
pdf("Plotting/Asset/Damage_ratios_comparison_2060.pdf")
bp_damage_ratios <- barplot(damage_ratios_benchmark_mat_2060, beside = TRUE, yaxt = "n", ylim = c(0, 0.4), col = gray(seq(0.95, 0, length.out = 5)), main = "Comparison of projected damage ratio (at 2060)")
axis(2, at = pretty(c(0, 0.3)), labels = paste0(pretty(c(0, 0.3)) * 100, "%"))
legend("topleft", legend = damage_ratios_benchmark_data_2060$Models, fill =  gray(seq(0.95, 0, length.out = 5)), cex = 0.7)
text(x = bp_damage_ratios, y = as.vector(damage_ratios_benchmark_mat_2060), labels = percent(as.vector(damage_ratios_benchmark_mat_2060), accuracy = 0.01), pos = 3, cex = 0.65, col = "black")
dev.off()
## Plot the damage-adjusted GDP: 
pdf("Plotting/Asset/AdjustedGDP_comparison_2060.pdf")
barplot(GDP_benchmark_mat_2060, beside = TRUE, col = gray(seq(0.95, 0, length.out = 6)), main = "Comparison of damage-adjusted GDP (at 2060)", ylim = c(0, 1.3e13))
abline(h = GDP_benchmark_data_2060[GDP_benchmark_data_2060$Models == "BHM-2015", ]$GDP_adjusted_SSP85, lty = 2, col = "red", lwd = 2)
legend("topleft", legend = GDP_benchmark_data_2060$Models, fill =  gray(seq(0.95, 0, length.out = 6)), cex = 0.7)
dev.off()
```
